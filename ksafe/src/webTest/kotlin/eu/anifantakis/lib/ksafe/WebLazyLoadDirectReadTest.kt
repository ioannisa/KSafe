...
/**
 * Locks in: on web `lazyLoad = true` must not disable the non-suspend read path. There is no
 * `runBlocking` here, so `ensureCacheReadyBlocking` cannot cold-load; suppress the collector too and
 * the cache stays empty all session, with every `getDirect` handing back the caller's default over
 * live persisted data — which a read-modify-write then overwrites.
 */
