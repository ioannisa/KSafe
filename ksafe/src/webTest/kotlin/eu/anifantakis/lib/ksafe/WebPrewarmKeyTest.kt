...
/**
 * Locks in: prewarming a web key must never mint one. Ciphertext (localStorage) and key (IndexedDB)
 * are independently clearable, so "key gone, data present" is the web's characteristic failure, and
 * the one-shot startup sweep reclaims it by classifying the decrypt probe's "web key missing" — a
 * minted master turns that probe into a GCM `OperationError`, which no orphan pattern matches.
 */
class WebPrewarmKeyTest {
    ...
    /**
     * End-to-end: an IndexedDB-only loss (DevTools clear, corruption reset, storage eviction) must
     * leave the surviving ciphertext reclaimable by the one-shot startup sweep.
     */
    @Test
