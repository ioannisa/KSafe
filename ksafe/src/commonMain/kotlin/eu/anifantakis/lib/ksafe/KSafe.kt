...
class KSafe @PublishedApi internal constructor(
     ...
     * re-creating `KSafe` mid-process. Waits briefly for a store write already running (not on
     * Web, which cannot block), so a store reopened right after sees it; queued writes are
     * dropped with a cancellation. Idempotent, and the instance is unusable afterwards.
     */
    fun close() {
        core.cancel()
    }
