...
import eu.anifantakis.lib.ksafe.internal.coreparts.incrementByOne
...
// close() blocks the (possibly main) caller thread; this plus the backend teardown bound stays
// under Android's ANR window.
internal const val CLOSE_QUIESCE_TIMEOUT_MS = 2_000L
...
internal class KSafeCore(
    ...
    /** Logical write queue — encryption happens inside the consumer, not on UI. A send to a parked
     *  consumer passes the element into that continuation, so a teardown landing in that window
     *  leaves it in neither the batch nor the channel; the handler is the awaiter's last exit. */
    internal val writeChannel = Channel<PendingWrite>(
        Channel.UNLIMITED,
        onUndeliveredElement = {
            it.completion?.cancel(
                CancellationException("KSafe was cancelled before this write was processed")
            )
        },
    )
    ...
    /** The orphan sweep's outcome, kept only so a diagnostic can tell "it threw" from "it never
     *  ran": the sweep is best-effort and swallows its failure. */
    internal val startupSweepFinished = KSafeAtomicFlag(false)

    @Volatile
    internal var startupSweepFailure: Throwable? = null
    ...
    /** Completed merges. No lock guards merging: a blocking cold reader that waited on the
     *  collector could hold the last free worker the collector needs to run, so readers merge on
     *  their own and use this count to tell when another merge COMPLETED around theirs. The tick
     *  lands after the merge, so one still in flight is invisible here; a non-lazy instance's
     *  collector re-merges on its next emission. */
    internal val mergeSequence = KSafeAtomicInt(0)
    ...
    internal suspend fun updateCache(
        ...
        while (true) {
            ...
            if (clearEpoch.get() == epoch || ++attempts >= 3) break
            epoch = clearEpoch.get()
            snap = storage.snapshot()
        }
        mergeSequence.incrementByOne()
    }

    /** Cold-cache preload for a caller that could not wait for the collector. If another merge landed
     *  around this one ([mergesSeenBeforeRead] is [mergeSequence] before the snapshot), the snapshot
     *  may predate it, so the store is read again: merging it last would resurrect swept entries,
     *  and dropping it would serve a default for a key the store holds. */
    internal suspend fun mergePreloadSnapshot(
        snapshot: Map<String, StoredValue>,
        epochAtSnapshot: Int,
        mergesSeenBeforeRead: Int,
    ) {
        var snap = snapshot
        var epoch = epochAtSnapshot
        var expected = mergesSeenBeforeRead
        var attempts = 0
        while (true) {
            updateCache(snap, epoch)
            expected += 1
            val landed = mergeSequence.get()
            if (landed == expected || ++attempts >= 3) return
            expected = landed
            epoch = clearEpoch.get()
            snap = storage.snapshot()
        }
    ...
    suspend fun ensureCacheReadySuspend() {
        ...
        // Epoch and merge count BEFORE the snapshot — the argument-order default would read after.
        val epoch = clearEpoch.get()
        val merges = mergeSequence.get()
        mergePreloadSnapshot(storage.snapshot(), epoch, merges)
        triggerLazyStartupCleanupOnce()
    }
    ...
    /** Set once [cancel] begins: no storage commit may start after it. */
    internal val closing = KSafeAtomicFlag(false)

    /** The storage commit running right now, if any; [cancel] waits for it before cancelling. */
    @Volatile
    internal var commitInFlight: CompletableDeferred<Unit>? = null

    /** Runs one storage commit under the window [cancel] waits on, or refuses it once closing. */
    internal suspend inline fun <T> commitToStorage(block: () -> T): T {
        val window = CompletableDeferred<Unit>()
        commitInFlight = window
        try {
            // Window published before the flag is read, so cancel() sees one or we see the other.
            if (closing.get()) throw CancellationException("KSafe was closed before this commit started")
            return block()
        } finally {
    ...
    /** Bounded, so a hung storage cannot hang shutdown; web cannot block and keeps cancelling at once. */
    private fun awaitCommitInFlight() {
        val window = commitInFlight ?: return
        if (window.isCompleted) return
        swallowingNonCancellation { window.joinBlockingWithin(CLOSE_QUIESCE_TIMEOUT_MS) }
    }

    /** Refuses new commits, waits (bounded) for the one running, then cancels both scopes;
     *  idempotent, and afterwards the instance no longer processes puts or reads. */
    internal fun cancel() {
