...
/**
 * Atomic +1. Not [raiseToAtLeast]: a ceiling seeded by a separate read swallows the second of two
 * concurrent bumps, and a merge that did not count is a merge a cold reader cannot see.
 */
internal fun KSafeAtomicInt.incrementByOne() {
    while (true) {
        val seen = get()
        if (compareAndSet(seen, seen + 1)) return
    }
}
...
/**
 * Capped backoff (250ms doubling to 30s) for a transient decrypt failure: a device locked overnight
 * must not wake the key store every second.
 */
