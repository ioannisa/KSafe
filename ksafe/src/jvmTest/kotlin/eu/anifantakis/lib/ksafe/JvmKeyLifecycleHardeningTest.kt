...
/**
 * Locks in: the key-lifecycle hardening around clearAll and rotation — a write racing clearAll
 * commits under the reset generation and stays rotatable, a stale pre-clear merge cannot republish
 * wiped secrets, an encrypted→plain overwrite reclaims the superseded per-entry key, and rotation
 * metadata stays fresh under every memory policy.
 */
class JvmKeyLifecycleHardeningTest {
    ...
    fun stalePreClearMerge_cannotRepublishWipedSecrets() = runTest {
        ...
        // Replays the race deterministically: a merge whose snapshot and epoch (the counter starts at
        // 0 and the one clearAll above moved it to 1) predate the wipe, landing after clearAll
        // returned. The epoch mismatch must make it redo from the post-clear snapshot.
