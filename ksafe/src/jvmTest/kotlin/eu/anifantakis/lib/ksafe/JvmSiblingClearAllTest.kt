...
/**
 * Locks in: `clearAll()` on one instance also wipes the in-memory copies every other live instance
 * on the same physical store holds — primary cache, plaintext side cache, routing metadata and key
 * generation — so a wiped secret can never be read back from a sibling for the rest of the process.
 */
class JvmSiblingClearAllTest {
    ...
    /**
     * A write staged in RAM but uncommitted when the wipe lands may end up on either side of it,
     * never both: whatever survives on disk must be exactly what the sibling serves from RAM.
     */
    @Test
    ...
    /**
     * A lazyLoad instance runs no collector, so a sibling's rotations never reach its cached
     * generation. The wipe must still reclaim every master the store minted.
     */
    @Test
    fun lazyLoadWiper_reclaimsTheMastersASiblingsRotationsMinted() = runBlocking {
        val engine = StatefulFakeEncryption()
        val a = KSafe(fileName = "sibmaster", baseDir = tmp, lazyLoad = true, testEngine = engine)
        val b = KSafe(fileName = "sibmaster", baseDir = tmp, testEngine = engine)
        try {
            assertRealSiblings(a, b)
            // Warms a while the store still holds nothing encrypted: it has nothing to adopt later.
            a.put("plain", "p", KSafeWriteMode.Plain)
            a.get("plain", "")
            b.put("token", "secret", KSafeWriteMode.Encrypted())
            b.rotateKeys()
            b.rotateKeys()
            assertEquals(3, b.core.currentKeyGeneration.get(), "precondition: the store rotated twice")
            assertEquals(1, a.core.currentKeyGeneration.get(), "precondition: the wiper's view lagged")
            assertTrue(a.core.cacheInitialized.get(), "precondition: the wiper never re-reads the store")

            a.clearAll()

            assertTrue(
                engine.liveAliases().none { "__ksafe_master__" in it },
                "a master minted by a sibling's rotation survived the wipe, got ${engine.liveAliases()}",
            )
        } finally {
            a.close(); b.close()
        }
