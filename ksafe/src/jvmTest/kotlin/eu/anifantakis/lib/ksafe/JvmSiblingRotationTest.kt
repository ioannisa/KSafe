...
class JvmSiblingRotationTest {
    ...
    /**
     * A sibling serving an older copy (its own write, kept by the dirty flag) must keep serving
     * it — that staleness is by design; degrading it into the default value is not.
     */
    @Test
    ...
    /**
     * The legacy twin: an `encrypted_`-prefixed record carries no metadata, so the copy rides a
     * per-entry alias only its protection literal names — the reclaim must count it as in use.
     */
    @Test
