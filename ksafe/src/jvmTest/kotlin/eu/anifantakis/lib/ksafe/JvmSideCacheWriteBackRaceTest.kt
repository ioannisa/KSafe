...
/**
 * Locks in: resolveFromCache's post-decrypt write-back must not overwrite a fresher value a racing
 * put placed in the side cache mid-decrypt — it is valid only while the primary cache still holds
 * the exact ciphertext that was decrypted, including when the put lands after the guard.
 */
class JvmSideCacheWriteBackRaceTest {
    ...
    private fun runScenario(policy: KSafeMemoryPolicy) {
        ...
        // This read is what triggers the decrypt, and with it the racing put; its own "old"
        // result is a legitimate point-in-time read, the next one is not.
