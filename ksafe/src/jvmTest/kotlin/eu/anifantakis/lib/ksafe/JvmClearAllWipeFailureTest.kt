...
class JvmClearAllWipeFailureTest {
    ...
    /**
     * The twin of the rollback cases: the wipe succeeds, and the `putDirect` that landed while it
     * was in flight is restored by the owner-gated putIfAbsent repair once its own batch commits.
     */
    @Test
    fun putLandingWhileTheWipeIsInFlight_isRepairedAfterASuccessfulClear() = runBlocking {
        ...
        // FIFO with one consumer: when this awaited write returns, the racing put has committed
        // and run its post-commit repair.
