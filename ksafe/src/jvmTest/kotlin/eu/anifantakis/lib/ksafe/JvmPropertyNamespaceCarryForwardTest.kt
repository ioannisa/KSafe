 ...
 * keys when a later release adds an explicit [KSafeConfig.appNamespace]. The config tier outranks
 * those, so without the shadow probe every alias misses, every decrypt reports "No encryption key
 * found", and the startup orphan sweep deletes ciphertext whose keys sit intact under the old name.
 */
class JvmPropertyNamespaceCarryForwardTest {
    ...
    fun withoutAConfigOverride_thereIsNothingToProbe() = withAppNamespaceProperty("acme") {
        // No override => the property tier is already the current namespace.
