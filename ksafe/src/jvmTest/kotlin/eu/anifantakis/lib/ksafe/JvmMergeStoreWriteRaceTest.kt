...
/**
 * Locks in: a write landing between the snapshot merge's dirty check and its cache store keeps its
 * value — the merge stores only while the slot still holds what it observed, so the racing write is
 * not pinned under the stale disk value for the rest of the process. Covers all three merge stores
...
class JvmMergeStoreWriteRaceTest {
    ...
    /**
     * A plain rewrite landing mid-merge drops the key's encryption metadata, which the merge must
     * not put back. The first updateCache gives the raced second one a value to CAS against.
     */
    @Test
