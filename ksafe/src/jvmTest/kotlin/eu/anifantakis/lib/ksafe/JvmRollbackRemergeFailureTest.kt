...
/**
 * Locks in: when the rollback's own re-merge read also fails — the usual case, since one disk fault
 * blinds both the write and the read after it — the never-persisted value must not stay in RAM.
 * Under `lazyLoad` nothing else repairs it, so reads would serve a phantom (plaintext, in the
 * encrypted slot) for the whole process lifetime.
 */
class JvmRollbackRemergeFailureTest {
    ...
    private class FaultyStorage : KSafePlatformStorage {
        ...
        /** How many upcoming [snapshot] calls to fail. */
    ...
    private fun buildCore(
        ...
        // Otherwise the startup sweep's own background read consumes the faults aimed at the write.
    ...
    /** Seeds a durable value, so the failing write below overwrites something the store holds. */
    ...
    /** (b) The generation read opening an encrypting batch fails, so the re-merge read fails too. */
    ...
    fun aNewerWriteClaimingTheKeyMidRollback_isNotDropped() = runBlocking {
        ...
        // Parking the newer write's batch keeps its optimistic state unrepaired for the assertions.
    ...
    /**
     * (e) A failed DELETE staged no value and no metadata, so the drop has nothing of its own to
     * take: the slots stay as the delete left them and disk still wins.
     */
    @Test
    ...
    /**
     * (f) A newer PLAIN write reclaims a failed encrypted key mid-rollback: the metadata drop is
     * CAS'd on the failed write's literals, so the plain write's own metadata must survive.
     */
    @Test
