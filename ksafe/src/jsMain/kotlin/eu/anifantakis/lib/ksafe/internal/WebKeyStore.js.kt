...
/**
 * Kotlin/JS actuals for [webKeyEnsure] et al. The WebCrypto + IndexedDB helper is a self-contained
 * JS IIFE returning a dispatcher function. The `js(...)` string references no Kotlin identifiers —
 * the Kotlin/JS IR compiler renames parameters, so referencing them inside `js(...)` fails at
 * runtime. Kept behaviourally identical to the wasmJs `@JsFun` variant.
 */
private val dispatch: (String, String, String?) -> Promise<Any?> = js(
    ...
    (function() {
      ...
      return function(op, a, b) {
        if (!impl) impl = init();
        if (op === 'ensure') return impl.ensure(a, b, true);
...
internal actual suspend fun webKeyEnsure(idbName: String, legacyRawKeyB64: String?, mintIfAbsent: Boolean) {
    dispatch(if (mintIfAbsent) "ensure" else "ensureNoMint", idbName, legacyRawKeyB64).await()
}
