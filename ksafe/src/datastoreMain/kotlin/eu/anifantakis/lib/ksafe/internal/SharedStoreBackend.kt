...
// Only DataStore's tail after the commit close() already waited for; the wait blocks the
// (possibly main) caller thread, and both bounds together stay under Android's ANR window.
internal const val BACKEND_TEARDOWN_TIMEOUT_MS = 1_000L
...
internal class SharedBackendRegistry<T : SharedStoreBackend>(
    ...
    fun acquire(path: String, create: (CoroutineScope) -> T): T = pathLock(path).withLock {
        ...
        terminatingScopes.remove(path)?.coroutineContext?.get(Job)?.let { priorJob ->
            if (!priorJob.joinBlockingWithin(BACKEND_TEARDOWN_TIMEOUT_MS)) onTeardownTimeout(path)
        }
    ...
    // The last release cancels the scope and waits (bounded) for DataStore's writer to finish the
    // rewrite it may be inside; a scope still busy is parked for the next acquire to await again.
    fun release(path: String): Unit = pathLock(path).withLock {
        ...
        if (backend.refCount <= 0) {
            ...
            val job = backend.scope.coroutineContext[Job]
            if (job != null && !job.joinBlockingWithin(BACKEND_TEARDOWN_TIMEOUT_MS)) {
