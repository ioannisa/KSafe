...
class MacosMultiInstanceTest {
    ...
    fun twoLiveInstances_sameFile_bothReadAndWrite() = runBlocking {
        ...
        a.put("ka", "va")
        // Awaited, not asserted outright: a sibling learns of the write through its own collector
        // on the shared store, so `a.put` returning means durable, not merged — asserting at once
        // passes only while the scheduler is idle. The timeout still fails a real regression.
        assertEquals(
            "va",
    ...
    fun twoInstances_sameFile_shareOneStorage() = runBlocking {
        ...
        try {
            // A private storage per instance means a private commit relay — commits would not cross.
            assertSame(
                a.core.storage,
                b.core.storage,
