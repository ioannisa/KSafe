...
/**
 * Locks in: [AppleKeychainEncryption.updateKeyAccessibility] skips the `SecItemUpdate` IPC when the
 * target accessibility policy equals the last one applied this process. The live SecItemUpdate
 * cannot run in a sandboxed test, so only the pure guard decision is verified here.
 */
class MacosAccessibilityUpdateGuardTest {

    @Test
    fun updateNeeded_onlyWhenPolicyChangesFromLastApplied() {
        // Not yet applied this process → must update.
