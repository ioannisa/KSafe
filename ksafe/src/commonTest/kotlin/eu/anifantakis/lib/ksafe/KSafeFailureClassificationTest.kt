 ...
 * (the orphan sweep deletes the ciphertext) and which mean "try again later". Both verdicts are
 * read off a message that interpolates the caller's key, so the definitive phrases are recognised
 * only in the canonical `"KSafe: <phrase>"` opening — else a key named after one sets its own verdict.
 */
