...
/**
 * Locks in: the generation-suffixed per-entry alias namespace is injective and master-sentinel key
 * names are reserved. A plain `.gN` suffix let user key `"foo"` at generation 2 collide with user
 * key `"foo.g2"` at generation 1 — rotating or deleting one destroyed the other's vault key — and
 * let a default store's dotted key alias a named store's key; the fingerprint closes both.
 */
class KSafeAliasNamespaceTest {
    ...
    fun rotatedAlias_neverEqualsA_fingerprintBearingTwinsBareAlias() {
        // The fingerprint alone was not injective: user key "foo.g2.h<fp(ns,foo)>" spelled "foo"'s
        // rotated alias exactly as its own generation-1 bare alias, two logical entries on one
        // physical vault key. The `__ksafe_gen__` segment, barred from user keys, closes the class.
    ...
    fun jvmVaultMarkerSuffixedKeys_areRejectedForWrites() {
        // The JVM tombstone ("$alias.__ksafe_nsdel__") and the software-fallback mint marker share
        // the per-entry vault-alias plane, so such a key aliases a sibling key's marker slot.
        assertFailsWith<IllegalArgumentException> {
