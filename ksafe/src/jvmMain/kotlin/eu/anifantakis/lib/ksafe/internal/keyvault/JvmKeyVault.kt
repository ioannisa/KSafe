...
import eu.anifantakis.lib.ksafe.internal.retryingTransientRename
...
internal class DataStorePrefStore(
    ...
    fun putString(alias: String, value: String) {
        ...
        runBlocking { retryingTransientRename { dataStore.edit { it[pref] = value } } }
    }

    fun remove(alias: String) {
        val pref = prefKey(alias)
        runBlocking { retryingTransientRename { dataStore.edit { it.remove(pref) } } }
    }
...
internal class JvmKeyVaultProvider(
    ...
    /**
     * Custody moved because the bridge died, not because anyone asked: a key minted now is
     * provisional, since the unreachable OS vault may still hold the real one for that alias.
     */
    val degradedByLinkFailure: Boolean
        get() = degraded.get()

    /** Deliberate `-Dksafe.jvm.keyVault=software`: this session owns every key it resolves here. */
    val softwareOptOutRequested: Boolean
        get() = softwareOptOut.get()
