...
**Major performance improvements** — the release headline: a new per-datastore master-key envelope (v2) eliminates per-entry Keystore/Keychain IPC on every encrypted read and write, with the largest gains on stores that hold many encrypted entries. Also bundles the KMP refactor, new platforms (macOS, Kotlin JS), and biometrics extracted into its own module. Cumulative of 2.0.0-RC1, 2.0.0-RC2, and the 2.0.0 release work below.

### Added

- **`KSafeMemoryPolicy.LAZY_PLAIN_TEXT` — new default memory policy on Android, iOS, macOS, and JVM.** Cold start keeps encrypted entries as Base64 ciphertext in the cache (no bulk decrypt), exactly like `ENCRYPTED`. The first read of each key decrypts on demand and caches the plaintext **permanently** in the secondary plaintext cache; every subsequent read is an O(1) memory lookup, exactly like `PLAIN_TEXT`. The result is the cheapest cold start *and* the fastest steady-state reads, with the same in-RAM exposure as `PLAIN_TEXT` after first read. Apps that touch a subset of keys never pay for the rest. Implemented on top of the existing `plaintextCache` infrastructure with the TTL check short-circuited to "never expire" for this policy. Web stays forced to `PLAIN_TEXT` (WebCrypto is async-only and `getDirect` is sync, so lazy decrypt isn't possible there).
- **Native macOS targets** across all three modules (`:ksafe`, `:ksafe-compose`, `:ksafe-biometrics`) ([#26](https://github.com/ioannisa/KSafe/issues/26), thanks @tomasjablonskis). `macosX64` + `macosArm64` artifacts use the same Keychain + CryptoKit + Secure Enclave path as iOS via a shared `appleMain` source set; DataStore-backed on-disk store at `NSApplicationSupportDirectory`. Intel Macs without a T2 chip fall back to plain Keychain storage automatically.
...
- **`allowDeviceCredentialFallback` parameter on `verifyBiometric` / `verifyBiometricDirect`** ([#29](https://github.com/ioannisa/KSafe/issues/29), thanks @Trucodisparo — partial fix: covers the disable-device-credential case from the issue, not the full `BiometricManager.Authenticators` bitmask). New optional `Boolean` parameter (default `true`). When `false`, restricts authentication to biometrics only — no PIN/password/pattern fallback is offered. Android uses `BIOMETRIC_STRONG` without `DEVICE_CREDENTIAL` and auto-adds a Cancel button (required by `BiometricPrompt`); Apple platforms switch to `LAPolicyDeviceOwnerAuthenticationWithBiometrics`; JVM/JS/WasmJS ignore it.
...
- **Compatibility with `dev.whyoleg.cryptography` 0.6.0** ([#27](https://github.com/ioannisa/KSafe/issues/27), [#30](https://github.com/ioannisa/KSafe/issues/30), thanks @HarukeyUA @chirag38-unity, via [#28](https://github.com/ioannisa/KSafe/pull/28)). Bumped `cryptography-kotlin` from 0.5.0 → 0.6.0. Resolves the runtime `IrLinkageError` on iOS when a consumer app pulled cryptography-kotlin 0.6.0 transitively (`KeyDecoder` became a typealias for `Decoder` in 0.6.0, breaking the previously-linked symbol). Kotlin was bumped alongside as required by the new cryptography release.
...
- **macOS biometrics now work on all Macs.** `KSafeBiometrics` previously used `LAPolicyDeviceOwnerAuthenticationWithBiometrics` on every Apple platform. On macOS that policy silently fails with `LAErrorBiometryNotAvailable` on Macs without a Touch ID sensor (Mac mini, many Intel MacBooks without T2 chip, etc.), resulting in `verifyBiometric` returning `false` with no prompt shown. macOS now uses `LAPolicyDeviceOwnerAuthentication`, which covers Touch ID where available and falls back to the macOS login password or Apple Watch unlock — guaranteeing a real prompt on every Mac. iOS is unchanged (`LAPolicyDeviceOwnerAuthenticationWithBiometrics`). Policy selection is done at runtime via `Platform.osFamily` so the shared `appleMain` source set is retained.
- **`verifyBiometric` (suspend variant) now dispatches `LAContext.evaluatePolicy` on the Main dispatcher** on Apple platforms, matching the existing behaviour of `verifyBiometricDirect`. Previously the evaluation could be initiated from whichever dispatcher the caller was suspended on.

### Changed

- **Default memory policy is now `LAZY_PLAIN_TEXT` on Android, iOS, macOS, and JVM** (was `ENCRYPTED`). Existing code that did not explicitly set `memoryPolicy` will see: identical cold-start time, faster reads after first access for each key (steady-state O(1) instead of one decrypt per read), and the same in-RAM plaintext exposure as `PLAIN_TEXT` for keys that have been read at least once. Apps that need ciphertext-at-rest semantics for sensitive values must now opt in explicitly with `memoryPolicy = KSafeMemoryPolicy.ENCRYPTED` or `ENCRYPTED_WITH_TIMED_CACHE`. Web's forced `PLAIN_TEXT` is unchanged.
- **`PLAIN_TEXT` memory policy is now discouraged in KDoc.** Its eager-decrypt-everything cold-start pays $O(n)$ Keystore round-trips up front (parallelised eight at a time, but still serial-IPC-bound on large stores), which can push first-read latency into ANR territory on Android with thousands of encrypted keys. `LAZY_PLAIN_TEXT` matches its steady-state read performance with cheap cold start. `PLAIN_TEXT` is still supported for callers who want decrypt failures surfaced synchronously at startup.
...
- **v2 envelope: per-datastore master key for non-isolated encrypted entries.** Pre-2.0 KSafe generated one Keystore/Keychain key per user key — every encrypt and decrypt paid a fresh round-trip across Binder (Android), `securityd` (Apple), or DataStore Preferences (JVM), and SE-wrapped entries on Apple paid an additional `SecKeyCreateDecryptedData` ECIES unwrap on top. 2.0 introduces an envelope marker (`"v":2`) on metadata and routes every new `KSafeProtection.DEFAULT` write through one of two AES-256 keys per datastore — a relaxed-accessibility variant and a `requireUnlockedDevice = true` variant — both eagerly created at `KSafe(...)` construction and held in-process by the engine's existing per-alias key cache after first unwrap. After construction settles, every subsequent encrypt and decrypt is a pure-CPU AES-GCM operation against in-RAM key bytes; no further IPC for the lifetime of the process. `KSafeProtection.HARDWARE_ISOLATED` writes still get a per-entry key — StrongBox / Secure Enclave isolation is the entire point and sharing would defeat it. **Compatibility**: existing on-disk entries (`"v":1` or legacy literal metadata) continue to read through the per-entry-key path unchanged; no migration, no rewrite. Mixing v1 and v2 entries in the same datastore is supported indefinitely. Entries written by 2.0 cannot be read by 1.x.
- **Parallel encryption inside `KSafeCore.processBatch`.** Encrypted writes in a batch are now deduplicated by user-key and run concurrently inside a `coroutineScope { … }` with a `Semaphore(8)` cap. Hardware-keystore IPC pipelines instead of running serially; `ENCRYPTED` memory policy no longer pays a write-time penalty over `PLAIN_TEXT`. With the v2 envelope above, `DEFAULT` entries in the same batch now share the master alias (no IPC at all after warm-up); `HARDWARE_ISOLATED` entries retain the IPC-pipelining win.
...
- **`AppleKeychainEncryption` in-process key-byte cache.** New `keyBytesCache: KSafeConcurrentMap<ByteArray>` keyed by user `keyId` holds the unwrapped raw AES key bytes. Both `getOrCreateKeychainKey` (encrypt) and `getExistingKeychainKey` (decrypt) consult it before any Keychain IPC; `deleteKey` invalidates. Cache hits short-circuit `SecItemCopyMatching` *and*, for Secure-Enclave-wrapped entries, the additional `SecKeyCreateDecryptedData` ECIES unwrap. **Where this helps:** repeat encrypt/decrypt on the *same* key within a single process — typical patterns include `ENCRYPTED` memory policy reads of the same persisted value during Compose recomposition (first read fetches from Keychain; every subsequent read is RAM-only), and repeat writes to the same key (the per-call key-fetch IPC is paid once instead of N times). **Where it does not help:** cold-start preload (every entry's alias is unique → cache miss for each), single-access workloads (first call always hits Keychain by definition), and `PLAIN_TEXT` memory policy reads after preload (those serve directly from the plaintext cache and don't touch the engine at all). Brings the Apple engine in line with the per-alias `SecretKey` handle caches the Android and JVM engines already had — Apple was the outlier.
- **Suspend `put`/`delete` now go through the write coalescer.** Previously they bypassed the channel and called `storage.applyBatch` per-call, so N concurrent suspend writes paid N full transactions. They now enqueue `PendingWrite.*` with a `CompletableDeferred` and `await()` their batch's commit. The consumer uses a greedy-drain (`tryReceive` until empty) plus an optional 16 ms window only when no caller is waiting, so sequential single-call suspend `put` doesn't pay the window cost. In benchmarks of 500 concurrent suspend writes, per-op latency drops 5–27× depending on the encryption mode.
- **`KSafeAtomicFlag hasAnyEncryptedKey`** short-circuits `detectProtection` for plain-only stores. Reads on stores with no encrypted entries skip the per-call `protectionMap` lookup entirely.
- **Benchmark refresh.** Numbers in [`docs/BENCHMARKS.md`](docs/BENCHMARKS.md) re-measured (median of 4 runs on a Galaxy S24). Suspend-API cells now exercise concurrent coroutines (`GlobalScope.launch` + `joinAll`) instead of sequential awaits, reflecting real-world usage where the coalescer is the point.

### Tooling

- **Migrated to Android Gradle Plugin 9.2 + Gradle 9.4.** AGP `8.13.1 → 9.2.1`, Gradle `8.14.4 → 9.4.1`. The `:ksafe` module — last on legacy `com.android.library` — now uses the AGP 9 KMP library plugin (`com.android.kotlin.multiplatform.library`), aligning all three modules. The standalone top-level `android { … }` block was folded into `kotlin { android { … } }` per the AGP 9 KMP DSL; Android resource handling, host-test config, and device-test config all moved to `withHostTestBuilder` / `withDeviceTestBuilder`. Source set `src/androidInstrumentedTest` was renamed to `src/androidDeviceTest` (the new KMP convention — old name is still aliased internally). Removed obsolete `gradle.properties` flags now defaulted in AGP 9: `android.useAndroidX`, `android.nonTransitiveRClass`, `kotlin.kmp.isolated-projects.support` (the last one is unsupported in Kotlin 2.2+).
