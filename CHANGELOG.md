...
- **Android & Apple biometrics: a biometric success that arrives after the caller was cancelled no longer seeds the authorization-session cache (FEEDBACK_4 low).** A prompt can succeed at the very moment the caller's coroutine is cancelled (the user navigated away / the screen was destroyed). Seeding the session cache then would grant a *later* call a prompt-free pass off an authorization the original caller never received — a biometric-gate bypass. A shared `seedBiometricSessionIfActive` guard now seeds only while the coroutine is still active (Android via `ensureActive`; Apple mirrors its existing continuation-active gate), so a cancelled auth is not cached and the cancellation propagates.
- **All platforms: a strict (`requireUnlockedDevice`) value is no longer readable from RAM during the optimistic write window (FEEDBACK_4 FB3-M1).** A strict write puts the plaintext into `memoryCache` optimistically and marks the entry strict, then the write consumer swaps it to ciphertext after the batch commits. In the window before that swap, a read fell through to the cached plaintext (the decrypt attempt fails non-transiently — plaintext isn't valid base64 — and the fallback returned the cached value), so on a locked device the secret was briefly readable straight from RAM, bypassing the unlock requirement. A strict read now returns the default whenever native-decrypt did not succeed, never the transiently-cached plaintext. (`getFlow` was already safe — it reads the on-disk snapshot, not the optimistic cache.)
- **All platforms: a transient decrypt failure on a locked device no longer throws out of `getDirect`, the value delegate, or StateFlow/Compose seeding (FEEDBACK_4 M-B / M-H).** `getDirect` is documented to return the caller's default when decryption fails, but a transient keystore/keychain failure (locked device, busy Keystore) was rethrown out of the non-suspend read path — crashing `by`-property access, `getStateFlow` construction, and Compose composition on a locked device. The non-suspend read path now returns the default on a transient failure; the suspend `get()` path still throws so coroutine callers can await unlock and retry (the intentional, documented asymmetry). A single guard at the shared choke point covers `getDirect`, the `by ksafe(...)` value delegate, the `getStateFlow` seed, and Compose seeding.
- **Compose / StateFlow: a stale flow emission that races the setter no longer permanently clobbers a user write (FEEDBACK_4 M-F).** The user-write echo guard's check-then-apply in `updateFromFlow` isn't atomic against the setter, so a stale emission that read the guard as clear just before the setter armed it could apply the pre-write value *after* the setter ran. Because the observed flow is `distinctUntilChanged`, the user's value was never re-emitted, so the clobber was **permanent** (stuck until an unrelated external change) — strictly worse than the accepted transient flicker. The matched-echo branch now re-applies the echoed (user) value, self-healing the clobber; in the non-raced case it writes a value the state already holds, which Compose/StateFlow treat as a no-op. Applied to both the Compose `KSafeComposeState` and the core `KSafeMutableStateFlow` twin.
- **Android: the first encrypted read no longer risks an ANR by reading the software DEK on the caller thread (FEEDBACK_4 low, partial).** The relaxed-DEFAULT data-encryption key (DEK) was unwrapped lazily on the first encrypted read via a blocking DataStore round-trip on the caller thread — an ANR when that caller is the UI thread. KSafe now warms an already-persisted DEK into the in-process cache in the background at construction (`prewarmDekReadIfPresent`), so the first read is served from cache. It is strictly **read-only** — it never creates or persists a DEK, so an unencrypted-only safe still writes none — and runs on the background write scope, which `close()` cancels cleanly. (`getDirect`'s one-time cold-start cache block on the caller thread is unchanged — it is the documented trade-off behind its synchronous "instant read" contract.)
- **All platforms: `lazyLoad = true` no longer skips the one-time startup cleanup (FEEDBACK_4 low).** The legacy-key sweep (which moves plaintext key material out of the weak location — the JVM DataStore file / web localStorage — into the secure store), the orphaned-ciphertext sweep, and the Apple access-policy migration ran only in the background collector, which `lazyLoad` never starts. So a lazy instance left plaintext keys in the weak location and skipped the migrations indefinitely. The one-time cleanup now also runs once on a lazy instance's first access, deferred onto a background scope so it never blocks the read.
- **All platforms: the flow property delegates (`asFlow` / `asStateFlow` / `asWritableFlow` / `asMutableStateFlow`) no longer double-initialize under concurrent first access (FEEDBACK_4 M-G).** Each delegate lazily built its flow on first read via an unsynchronized read-check-then-assign on a plain field, so two threads hitting first access at once both ran the init block — a losing caller got a non-canonical instance, and for the observer-backed delegates a *second* observer coroutine was launched and leaked (re-decrypting on every external change). The delegates now use `@Volatile` fields with double-checked init under a lock (steady-state reads stay lock-free; only the one-time init serializes), so concurrent first access returns exactly one canonical flow and starts exactly one observer.
- **Core: a failing encrypted write no longer cross-contaminates a superseded same-key awaiter in the same batch (FEEDBACK_4 M-A).** The write coalescer merges multiple ops for one key into a last-write-wins batch, and only a key's final op is encrypted. When the final op is an encrypted write that fails (e.g. a `requireUnlockedDevice` write on a locked device) and an earlier op for the same key — a `delete(k)` or plain `put(k)` — shared the batch, the failure handler iterated the raw batch and completed that earlier op's awaiter with the encryption exception too, so an unrelated `delete`/plain-`put` caller received a keystore error. The handler now fails only the final (failing) op per key; superseded same-key ops are absorbed into the coalesced write and complete normally, exactly as they would if the final op had succeeded.
- **All platforms: a pre-2.0 flat plaintext user key that merely begins with `ksafe_` is no longer dropped on upgrade (FEEDBACK_4 M-C).** The internal-key filter that hides KSafe's own storage entries from the user-value view matched a blanket single-underscore `ksafe_` prefix, so a user key such as `ksafe_theme` stored before 2.0 (as a flat, non-enveloped plaintext entry) was classified as internal and silently dropped when the store loaded. The filter now matches only the genuine internal prefixes — the reserved `__ksafe_*` namespace and the web engine's `ksafe_key_*` key store — so real user keys survive while internal entries are still skipped.
- **macOS/Desktop (JVM): the login-Keychain key vault now fails closed on `errSecDuplicateItem` (FEEDBACK_4 low).** `put` is a delete-then-add upsert; it used to swallow `errSecDuplicateItem` from the add. But a duplicate error means the preceding delete didn't remove the existing item — so the *new* key was never stored and the stale one remained, while `put` reported success. Everything then re-encrypted under the intended key became undecryptable. `put` now throws on any non-success status (only `errSecSuccess` is a success), so the write fails closed instead of being silently lost.
- **JVM/Desktop: the software key vault (`FileKeyVault`) now fsyncs the parent directory after its atomic key-file write (FEEDBACK_4 low).** It already fsynced the temp file's data before the rename, but not the directory entry the rename creates — so a crash right after the move could lose the rename and leave the key file missing, which reads as "no keys yet" and lets the startup orphan sweep delete every encrypted entry. The parent directory is now fsynced after the move (best-effort; a no-op where a directory can't be opened as a channel, e.g. Windows, whose atomic move is the guarantee there).
- **JVM/Desktop: no residual live fallback secrets survive migration or `clearAll()` (FEEDBACK_4 low).** When the JSON→OS-backed migration's archive rename failed and it fell back to a copy, it left the *live* `<base>.ksafe.json` (ciphertext) and `<base>.ksafe-keys.json` (plaintext AES key) in place — the copy path now deletes the source after a successful copy, mirroring the rename path. And `clearAll()` on the OS-backed path now also sweeps any live `<base>.ksafe.json` / `<base>.ksafe-keys.json` a prior no-`Unsafe` period left behind (matched by exact name so a sibling safe's files are spared).
- **JVM/Desktop: `clearAll()` now also deletes the DataStore corruption-quarantine copies (FEEDBACK_4 M-E).** When the OS-backed `<base>.preferences_pb` is unreadable, DataStore's corruption handler quarantines it as `<base>.preferences_pb.corrupt-<ts>` — a copy that still holds decryptable ciphertext. `clearAll()` promises a full, key-deleting wipe, but its residual-file sweep only matched the `<base>.ksafe` (JSON-fallback) prefix, so the protobuf quarantine copies survived. `clearAll()` now removes them too, matched by the exact datastore file-name prefix so a sibling safe's files in the same directory are left untouched.
- **Apple: a strict (`requireUnlockedDevice`) key's plaintext no longer lingers in the in-process key cache (FEEDBACK_4 low).** A non-strict write caches the unwrapped AES key bytes for fast reads; if the same key was later rewritten strict, those bytes lingered — the plaintext of a key that should require an unlocked device stayed in RAM. A strict access (read or write) now evicts any cached entry and never repopulates it, upholding the invariant that a strict key's plaintext is never cached.
- **Apple (iOS/tvOS/watchOS): a named instance's Keychain orphan sweep can no longer delete a root instance's live key (FEEDBACK_4 M-D).** A Keychain account is `keyPrefix.fileName.userKey`, and user keys may contain dots, so a root (`KSafe()`) instance's `HARDWARE_ISOLATED` key named e.g. `"vault.token"` produces an account byte-identical to a named `KSafe(fileName = "vault")` instance's `"token"`. The named instance's sweep couldn't tell them apart and could reap — irrecoverably delete — the root instance's live key. A named instance now reaps only key-ids it can *prove* it owns (its live keys); a cross-session Keychain orphan it can't prove is its own is left as harmless unused-key clutter (reclaimed by `clearAll()`) instead of being auto-deleted. The root/no-`fileName` sweep is unchanged (its existing dotted-key guard already defers ambiguous ids to the named instances that own them).
- **Apple: a `HARDWARE_ISOLATED` write no longer fires three `SecItemUpdate` Keychain IPC round-trips on every write (FEEDBACK_4 FB3-M4).** Re-asserting `kSecAttrAccessible` on the key item, the SE-wrapped item, and the SE key is only needed on an actual unlock-policy CHANGE; a first-time create already stored the correct accessibility and a same-policy rewrite (the common case) needs none. An in-process cache of the last-applied policy per key now skips the IPC when it is unchanged, recording it only after all three updates succeed so a best-effort partial failure is still retried on the next write. (A genuine policy change is still re-asserted, and remains last-writer-wins per the write's explicit `requireUnlockedDevice`.)
- **Apple (iOS/tvOS/watchOS): the Keychain orphan sweep no longer deletes a key a concurrent write just re-used (FEEDBACK_4 H-B).** The sweep classifies orphans against a frozen DataStore snapshot on the collector scope, then deletes the corresponding Keychain items — but writes run on a separate scope, genuinely in parallel on Native. The in-flight guard (added to prevent exactly this) was consulted only at *classify* time, so a `put` that committed ciphertext and re-used a key *after* it was classified but *before* the delete had its live key destroyed, making the just-written value undecryptable. The guard is now re-checked at *delete* time as well (a write marks its key in-flight before its commit lands, so the re-check closes the window). macOS remains unaffected (the sweep is disabled there).
- **Core: the post-commit repair's Plain branch no longer resurrects orphan protection metadata for a key deleted mid-repair (FEEDBACK_4 FB3-M3).** The Plain twin of FB3-H2: a `delete()` racing a plain write's post-commit repair could leave `protectionMap` populated for the deleted key (so `getKeyInfo` returned non-null for it) because the coupled cleanup keyed off the now-absent value. The Plain branch now applies the same `!memoryCache.containsKey` orphan-drop as the Encrypted branch, and the suspend plain-put path caches its value before its protection literal so the value-before-protection invariant the cleanup relies on holds on both put paths.
- **Core: a `delete()` racing the post-commit repair no longer resurrects orphan metadata that permanently breaks `getOrCreateSecret`/`getKeyInfo` (FEEDBACK_4 FB3-H2).** After a committed encrypted write, the repair re-asserts the key's `memoryCache`/`protectionMap`/`encMetaMap` via three separate `putIfAbsent`s (so a `clearAll` that wiped the optimistic state can't make an acknowledged write read back as the default). Those inserts weren't atomic against a caller-thread `delete()`, which claims ownership and wipes all three maps: in the losing interleaving the value insert ran first as a no-op, the delete then wiped everything, and the protection/encMeta inserts **resurrected metadata for the now-deleted key**. The coupled cleanup keyed off the (now-absent) value, so it skipped the orphans — leaving `getKeyInfo` non-null for a deleted key and `getOrCreateSecret` throwing "refusing to overwrite" for the rest of the session (breaking delete-then-recreate / logout-relogin secret rotation; in-memory only, disk was correct). The repair now also drops exactly the literals it inserted when it lost ownership and **no value is cached** for the key — which cannot describe a live newer writer, since a put caches its value before its protection, so a newer write's (possibly identical-literal) metadata is still never clobbered.
- **All platforms: an external write is no longer masked by a stale plaintext side cache under `LAZY_PLAIN_TEXT` (the default) / `ENCRYPTED_WITH_TIMED_CACHE` (FEEDBACK_4 H-F).** `updateCache` — the only path that merges an *external* change (another KSafe instance, another process, or a direct DataStore edit) — refreshed the primary `memoryCache` but never the secondary `plaintextCache`. Under `LAZY_PLAIN_TEXT` that side cache never expires, so after an external write a reader kept returning the pre-change plaintext for the rest of the session: `getDirect`/`get` silently diverged from `getFlow`, and read-your-neighbour's-write was broken for the default policy (under `ENCRYPTED_WITH_TIMED_CACHE` the staleness lasted up to the TTL). `updateCache` now evicts the side-cache entry when the merged ciphertext actually changed (a fresh IV per encrypt means a real external write always differs, while an unrelated key's snapshot entry is byte-identical, so its side cache — and the performance benefit — is preserved), and mirrors an external delete into the side cache so a deleted secret's plaintext can't linger in the never-expiring cache. The next read re-decrypts the fresh value and repopulates the cache.
- **JVM/Desktop: a failed *archive* step in the JSON→OS-backed migration no longer silently reverts every later write on every launch (FEEDBACK_4 H-C).** The one-time migration commits the re-encrypted values to the OS store durably, then archives the JSON fallback to `*.migrated` as the "already migrated" signal the launch-time gate keys off. That archive was best-effort: if both the rename **and** the copy failed (permissions, AV lock, read-only/full directory), no marker was left, so the gate said "needs migration" on the next launch and re-drained the now-stale fallback over whatever the user had written in between — a perpetual, silent rollback. The archive step now guarantees a durable done-marker: rename → copy → and, if both are blocked, an empty 0-byte `*.migrated` sentinel (a successful migration has just written the OS store into the same directory, so the sentinel virtually always succeeds). The retry-safety `.migration-pending` state is cleared only once the migration is durably marked done, so a re-run can never roll back newer writes.
- **JVM/Desktop: adding `config.appNamespace` on upgrade no longer orphans the OS-vault keys (FEEDBACK_4 H-D).** Setting `appNamespace` re-homes the data file into a per-namespace subdirectory (and changes the OS-vault service namespace from the stable default `"shared"` to the app's namespace), but the encryption keys written before the namespace was configured stayed under `"shared"` — so every encrypted value became undecryptable. The existing legacy-namespace read-fallback (which already migrates keys forward on a miss) now also probes `"shared"` when an explicit `appNamespace` is in effect, so pre-`appNamespace` keys are migrated to the new namespace on first access and existing data survives the upgrade. (The JVM twin of the web FB3-H1 fix.)
- **Web (JS/Wasm): `getOrCreateSecret` with a secret larger than 64 KB no longer throws (FEEDBACK_4 low).** WebCrypto's `getRandomValues` rejects a view longer than 65536 bytes (`QuotaExceededError`), so `secureRandomBytes(size > 65536)` — reached when generating a large secret — failed. The web random source now fills in ≤64 KB chunks.
- **Web: constructing an `appNamespace`d store no longer cannibalizes a co-existing no-namespace store on the same `fileName` (FEEDBACK_4 FB3-M2).** The 2.1.4 upgrade migration that carries un-namespaced data forward into the `appNamespace`d prefix ran on every construction (no done-marker), and its source prefix `ksafe.<file>:` is exactly the *live* localStorage prefix of a co-existing no-namespace `KSafe(fileName=<file>)`. Its copy→verify→**delete** therefore stole that sibling's entries — including fresh writes — on each launch. The un-namespaced migration is now non-destructive (copy-if-absent, no source delete), matching the already-non-destructive key migration (FB3-H1); the namespaced store scopes its own reads/`clearAll` strictly to its prefix, so the harmless orphaned copy never bleeds across. The legacy `ksafe_<name>_` migration still deletes its (retired, no-live-writer) source.
- **Web: adding `appNamespace` on upgrade no longer orphans the encryption key (FEEDBACK_4 FB3-H1).** The 2.1.4 `appNamespace` data-store isolation migrated the ciphertext to the namespaced `localStorage` prefix, but the AES-GCM `CryptoKey`'s IndexedDB record name is derived independently (it already incorporated `appNamespace`), so an existing app that added `appNamespace` on upgrade found no key at the new namespaced name and its encrypted data became unreadable. The engine now migrates a pre-`appNamespace` key forward to its namespaced record name on first access (non-destructive — copies only if the destination is absent), so existing encrypted data survives the upgrade. (Combined with the H-A fix, an unmigrated key can no longer be silently overwritten either.)
- **Web: values written with `requireUnlockedDevice = true` are now readable (FEEDBACK_4 H-E).** Web forces the `PLAIN_TEXT` memory policy and has no device-lock to bind the flag to, but the strict read path routed to the engine's **blocking** `decrypt`, which the async-only WebCrypto engine throws `UnsupportedOperationException` from — so a strict value could be written but never read back (`get`/`getDirect` threw). The web factory now strips `requireUnlockedDevice` (it cannot be hardware-enforced in a browser), so such values read back normally. On-disk format for web is unaffected (web data is origin-local and never read by other platforms).
...
### Fixed — full-library audit (2026-07-03)

A fresh full-library audit of the 2.1.4 tree confirmed the release is net-safer than 2.1.3 (no reverting) and surfaced residual edges — including two carried in by the FEEDBACK_4 fixes themselves. Fixed before release:

- **JVM/Desktop: adding `appNamespace` no longer lets one instance destructively steal a co-existing no-namespace instance's OS-vault key (FULL_AUDIT H2).** The FEEDBACK_4 H-D fix made a namespaced instance probe the stable-default `"shared"` namespace as a read-fallback so its own pre-namespace keys migrate forward on upgrade. But `"shared"` is also the live active namespace of any co-existing `KSafe()` (no-`appNamespace`) instance on the same host, and the recovery performed a **MOVE** — copy-then-`delete` of the source entry — so probing it deleted the sibling's live key and permanently orphaned that instance's ciphertext (the delete-time scrub of the legacy twin on `deleteKey` had the same reach). Recovery from the `"shared"` source is now **copy-only**; the destructive delete is reserved for a genuine 2.1.0/2.1.1 **derived** legacy namespace, which has no live owner. (Residual of the H-D fix; the only cost is a harmless lingering `"shared"` entry, reclaimed on an explicit `deleteKey` of that instance's own key.)
- **Apple (iOS/macOS): the flow-delegate lazy init no longer serializes every delegate on one process-wide busy-spin lock held across blocking I/O (FULL_AUDIT M11).** The FEEDBACK_4 M-G double-checked-init fix used, on Kotlin/Native, a single global spin lock — so (a) an unrelated delegate's cold-start read (a blocking DataStore/keychain round-trip that runs *under* the lock) blocked the first access of every OTHER flow delegate, and (b) a waiting thread busy-spun a CPU core instead of parking. Each delegate now owns its own reentrant `NSRecursiveLock` — a real OS parking lock (JVM/Android use `ReentrantLock`; single-threaded web is unchanged): waiters block instead of spin, unrelated delegates never contend, and a nested first-access on one thread can't self-deadlock. (Residual of the M-G fix; JVM/Android/web behaviour is unchanged — they already used a per-object parking monitor.)
- **Web (Kotlin/JS + Wasm): adding `appNamespace` no longer lets the first-constructed namespace destroy the shared legacy data for every other namespace of the same `fileName` (FULL_AUDIT H1).** The one-time migration that carries pre-namespace data forward from the flat `ksafe_<file>_` localStorage prefix ran with `deleteSource = true`, but that flat prefix has no appNamespace segment — it is a SHARED source across all namespaces of a `fileName`. With two same-`fileName` appNamespaced stores, whichever constructed first copied the legacy data to its own namespaced prefix and then DELETED the shared source, so every other namespace found nothing and read defaults for keys it owned. The legacy migration is now non-destructive whenever an `appNamespace` is set (copy-if-absent, source preserved — every namespace migrates from the still-present source), matching the un-namespaced FB3-M2 fix; the no-`appNamespace` path still reclaims the source (a single canonical store). (Twin of FB3-M2.)
- **Web (Kotlin/JS + Wasm): the pre-`appNamespace` CryptoKey record left by the FB3-H1 copy-forward migration is intentionally NOT deleted on `deleteKey`/`clearAll` — assessed and guarded (FULL_AUDIT L9).** The audit suggested cleaning up that orphaned IndexedDB record, but its record name is byte-identical to the key a co-existing no-`appNamespace` `KSafe(fileName = …)` on the same origin uses, so deleting it from a namespaced instance would destroy that live sibling's key — an H2-class cross-instance data loss, exactly what the non-destructive FB3-M2/FB3-H1 migrations protect against. The record is a non-extractable key with no plaintext exposure, so it is deliberately retained as a harmless orphan; a new regression test locks in that a namespaced `clearAll()` never breaks a co-existing no-namespace sibling's key.
- **JVM/Desktop: an app that upgraded from 2.1.0/2.1.1 (launcher-derived OS-vault namespace) AND newly set an explicit `appNamespace` no longer orphans its keys (FULL_AUDIT H3).** The FEEDBACK_4 H-D read-fallback probed a *single* legacy namespace — the launcher-derived namespace when on the stable default, OR `"shared"` when an explicit `appNamespace` was set. But a 2.1.0/2.1.1 stable-launcher app that *later* adopts an `appNamespace` holds its key under the **derived** namespace, which the explicit-namespace path never probed, so decrypt threw "No encryption key found" and the startup orphan sweep deleted the recoverable ciphertext. The recovery now probes a **set** of candidate namespaces (the derived namespace and `"shared"`, minus the active one), migrating from the first that holds the key — covering default→derived, explicit→shared, and explicit→derived. (Completes the H-D fix; the per-source delete gate from H2 still applies, so `"shared"` is never scrubbed.)
- **`getOrCreateSecret` no longer surfaces a raw keystore exception on a locked device (FULL_AUDIT M6).** The suspend `get()` path rethrows a *transient* decrypt failure (a locked device / momentarily-unavailable vault) rather than collapsing it to the default — the intentional, retryable asymmetry with `getDirect`, now documented on `get()`'s KDoc. But `getOrCreateSecret` assumed `get<String>(key, "")` always collapses an unreadable secret to `""`, so a locked-device call threw a raw keystore exception instead of its documented refuse-to-rotate `IllegalStateException`, and never reached the guard that prevents regenerating over a still-present secret. It now catches the transient failure and routes it to the same refuse-to-rotate error when the secret exists, so a locked device yields the helpful "temporarily unavailable — retry" message and the secret is never rotated.
- **`asMutableStateFlow` / `KSafeMutableStateFlow`: concurrent writers can no longer corrupt the write-echo guard (FULL_AUDIT M12).** `MutableStateFlow` permits concurrent writers, and the state's write-echo bookkeeping (`lastUserWrite`, `syncedValue`, `hasSynced`, and the guard flag) was mutated across four independent `@Volatile` fields with no serialization. Two interleaving writers could leave the guard's armed/disarmed decision paired with a *different* value than the one actually visible, so the echo of the visible value never matched and the guard stayed stuck — permanently suppressing later external changes. The write-side transition (the setter, `compareAndSet`, and the flow-observer callback) now runs under a per-instance lock, so a concurrent writer never observes a half-updated tuple; `persist()` stays outside the lock. (Residual of the M-F fix.)
- **JVM/Desktop: a transient "vault unavailable" during legacy-namespace recovery no longer reads as a genuine miss (FULL_AUDIT H4).** `recoverFromLegacyNamespace`'s probe read caught every `Throwable` and collapsed it to `null`, discarding the deliberate "vault unavailable" signal the OS vaults throw (never null) to distinguish *store unreachable* from *key absent*. A login keychain re-locked between round-trips, or a keyring not yet on D-Bus, during recovery was therefore misread as a genuine miss — so the startup orphan sweep deleted the still-recoverable ciphertext, or a fresh key was minted that permanently shadows the real one. The probe read now rethrows a "vault unavailable" error (as it already rethrows `LinkageError`), so the caller reports "vault unavailable" (which the orphan sweep treats as non-deletable) and the ciphertext survives until the store is reachable.
- **JVM/Desktop: a corrupt `.migration-pending` file no longer makes the JSON→OS-backed migration roll back the user's writes (FULL_AUDIT H5).** The presence of that file proves the current launch is a *retry* of a transiently-failed migration — during which the session ran on the OS-backed store and may have written newer values. If the file was left truncated (a partial non-atomic write interrupted by process death / a full disk), decoding it returned `null`, which reverted the migration to a *first attempt* — re-applying the frozen fallback values over every key and silently overwriting the user's newer writes. A present-but-corrupt pending file is now treated as a retry with an *unknown baseline*: any key the target already holds a value for is conservatively kept (the fallback copy stays recoverable in the `.migrated` archive), and only keys the target lacks are migrated. The pending file is also written atomically (temp + rename) so a partial write can't corrupt it in the first place.
- **Web (Kotlin/JS + Wasm): `KSafe()` and `KSafe(fileName = "default")` no longer let whichever constructs first destroy the other's pre-2.1.4 data (FULL_AUDIT H9).** Both mapped to the same on-disk prefix `ksafe_default_` before 2.1.4. The new scheme gives them distinct data prefixes (`ksafe.:` vs `ksafe.default:`) but derives the same legacy migration *source*, so the destructive migration let the first-constructed instance copy the shared legacy data to its own prefix and delete the source — stranding it for the other. The legacy migration is now non-destructive for the shared `ksafe_default_` prefix (copy-if-absent, source preserved), so both instances migrate the (ambiguous, previously-shared) data forward. (Same fix as H1.)
- **All platforms: an encrypted `getStateFlow` / `getFlow` no longer runs its per-emission decrypt on the collector's dispatcher, avoiding a main-thread ANR on Android (FULL_AUDIT H8).** `getFlowRaw`'s snapshot-decrypt `.map` had no `flowOn`, so when a `getStateFlow` was collected on a `viewModelScope` (`Dispatchers.Main.immediate`), every DataStore snapshot emission decrypted on the main thread — on Android a synchronous Binder round-trip to the Keystore daemon (~7.6 ms/op) — and several observed encrypted flows or frequent writes stacked those into an ANR / dropped frames. The chain now ends with `.flowOn(decryptFlowContext)` — `Dispatchers.Default` on Android/JVM/Apple (where the OS-vault decrypt is a blocking IPC call), a no-op on single-threaded web (async WebCrypto, nothing to off-load) — matching the suspend `getRaw` path's `withContext(Dispatchers.Default)`, so the blocking keystore IPC runs off the collector's dispatcher; emission ordering and `distinctUntilChanged` are preserved. (A written value now propagates to an observed StateFlow asynchronously — standard StateFlow semantics — rather than synchronously on the main thread.)
- **Compose (`mutableStateOf` / `rememberKSafeState`): the one-shot cold-start self-heal can no longer permanently clobber a concurrent user write (FULL_AUDIT H7).** On a key whose stored value equals the default (or a slow async decrypt), a background self-heal reads the persisted value and applies it via `updateFromStorage`. Its check-then-apply wasn't atomic against the setter: a user write that armed the write-echo guard between the check and the apply was overwritten with the stale persisted value — and because this is the one-shot `getFlow().first()` path (not a live `collect`), no later emission ever corrected it, so the visible state diverged from storage permanently. The setter now arms the guard *before* publishing its value, and `updateFromStorage` re-checks the guard after its write and re-applies the user's value if a write raced in (mirroring the M-F self-heal on the live-observe path). (JVM/Native only; single-threaded web never races.)
- **Apple (iOS/macOS): the Keychain query dictionaries no longer leak native memory on every operation (FULL_AUDIT H6).** Each Keychain query built a `CFDictionary` with **null** value-callbacks and inserted values via `CFBridgingRetain` (a +1 the caller owns). A null-callbacks dictionary does not retain its values, so releasing only the dictionary left every bridged value's +1 outstanding — and since a strict (`requireUnlockedDevice`) read bypasses the in-process key cache and hits the Keychain on **every** read, native memory grew one `CFString` pair per read without bound. Every `CFBridgingRetain` is now balanced with a `CFRelease` after the `SecItem*` call across all sites (the password- and SE-key query builders, SE key creation, `storeInKeychain`, and the orphan-cleanup probe). (iOS-specific; the JVM/Android/web engines were unaffected.)
- **`getOrCreateSecret` now derives an injective storage key, so distinct secret names can no longer silently collide into one secret (FULL_AUDIT M7).** The pre-2.1.4 scheme collapsed every non-`[A-Za-z0-9_]` character to `_`, so `getOrCreateSecret("main.db")` (the KDoc example) and `getOrCreateSecret()` (default `"main_db"`) — or any two names differing only in punctuation — aliased to one storage slot and silently shared/overwrote a single secret, orphaning whatever the first protected (e.g. a SQLCipher database). A name already in `[A-Za-z0-9_]` keeps its historical slot (no migration for the common case); any other name is hex-encoded under a distinct `ksafe_secretx_` prefix that — differing at a fixed position from the plain prefix — can never collide with a plain-name slot. A special-char name's pre-2.1.4 secret is migrated forward from the old (shared) slot on first access, **non-destructively** — the legacy slot is never deleted, since a co-existing safe-name sibling may own it.
