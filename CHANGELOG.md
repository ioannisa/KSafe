...
- **`AppleKeychainEncryption` in-process key-byte cache.** Closes the Keychain IPC gap that the Android and JVM engines already covered: every `encrypt`/`decrypt` previously re-fetched the raw AES key from the Keychain (and, for Secure-Enclave-wrapped entries, re-ran an ECIES unwrap) on every call. New `keyBytesCache: KSafeConcurrentMap<ByteArray>` keyed by user `keyId` holds the unwrapped raw AES key bytes; both `getOrCreateKeychainKey` (encrypt) and `getExistingKeychainKey` (decrypt) consult it before any Keychain IPC; `deleteKey` invalidates. Cache hits short-circuit `SecItemCopyMatching` *and* `SecKeyCreateDecryptedData` for SE-wrapped entries. A 500-key store under `PLAIN_TEXT` policy now performs 500 Keychain IPCs on first run and ~0 on every subsequent operation for the process lifetime; under `ENCRYPTED` policy, only the first read per alias hits the daemon. Brings the Apple engine in line with the per-alias `SecretKey` handle caches the Android and JVM engines already had — Apple was the outlier.
