...
- **`BiometricHelper.promptTitle` / `BiometricHelper.promptSubtitle` are gone.** They were Android-only globals that could not be reached from shared code and never influenced the web passkey name. Replaced by `KSafeBiometrics.defaultTitle` / `defaultReason`, which are common and feed every platform. Migration is a rename: `BiometricHelper.promptTitle = x` → `KSafeBiometrics.defaultTitle = x`, `BiometricHelper.promptSubtitle = x` → `KSafeBiometrics.defaultReason = x`. `BiometricHelper.confirmationRequired` is unchanged.
...
- **Compose state no longer loses a durable external change that lands during its own write's echo window.** Only the write's exact echo was accepted while the echo was pending; a genuine external value arriving in that window was consumed and dropped, and the deduplicated flow never re-emitted it — the state diverged from storage until some later different write. The stale pre-write snapshot alone is now suppressed; the echo or any real external change applies and clears the latch.
- **A failed fire-and-forget persist no longer leaves `asMutableStateFlow` — or a Compose `mutableStateOf` state — showing a value that never reached disk.** The persist path now reports the failure to the wrapper, which re-reads the durable value and reconciles (never reverting a newer write). The Compose delegates previously caught only a synchronous throw (and even then kept the phantom value); both the synchronous and the asynchronous failure now roll the optimistic state back. The Compose rollback keys off the write that owns the visible value rather than off the latch that suppresses stale emissions, so it also fires for a toggle flipped back to its stored value, for a failure arriving after the five-second echo window, and under a mutation policy that equates nothing — and a write that lands while a rollback is publishing survives it. When a failed persist cannot resolve a durable value, every wrapper now settles on the value it last had in sync with storage rather than on the caller's default: a strict entry on a locked device reads back as exactly the fallback the read was given, so the default published "empty" over data that is intact on disk — for a token, that read to the app as logged out. `mutableStateOf` and `asMutableStateFlow` re-read storage with that in-sync value as the fallback, so a fresher durable value still wins; `rememberKSafeState` reads through a memoized seam that binds the caller's default and so reverts to the in-sync value directly. The seam is public: a new `putDirect(key, value, mode, onWriteFailed)` overload notifies once the background persist fails, after KSafe rolled its caches back.
- **Two `asMutableStateFlow` writers racing the same key can no longer make the discarded value the durable one.** The flow published each write under its lock but enqueued the persist after releasing it, so writers could publish A→B yet persist B→A — the visible ordering said B won while A silently became the value on disk. The persist enqueue (which never blocks) now runs inside the same lock, so persist order always matches publish order.
- **`getOrCreateSecret` writes its provenance marker before the secret.** A crash between the secret and its owner marker left a marker-less live secret — the exact state that re-enables the legacy-name collision the marker was built to prevent. The order is now marker-first; the intermediate state is provably harmless.
- **Web migrations can no longer strand data.** The first namespaced instance imported the shared legacy encryption key into its own records and deleted the shared source, stranding sibling namespaces' ciphertext; the key is now durably imported into the shared record first and the source scrubbed only after verification. Migration completion markers are now written only after every copy verifiably succeeded, so a quota failure on one value retries instead of being sealed away. The cross-tab key-eviction broadcast now fires after the deletion completes, and encrypts verify the key record's survival before returning.
- **`clearBiometricAuth()` now revokes an in-flight prompt's authority.** A prompt already on screen when the clear ran could complete afterwards and re-seed the authorization cache, undoing a logout/lock revocation; sessions now carry revocation epochs captured before each prompt, so a post-revocation success authorizes that caller once but never seeds the prompt-free window. Web `resetRegistration()` also clears active authorization sessions, so the next verification really performs a fresh ceremony.
- **JVM Desktop prompts are serialized through the single-prompt gate (like Android and the web), and cancelling a caller dismisses the Windows Hello dialog.** Two concurrent `verifyBiometric` calls could stack two OS dialogs — or the loser was denied as `DeviceBusy` — and a queued caller re-prompted even when the caller ahead had just authorized the same scope; callers now queue and re-check the authorization cache once the gate is held. Cancelling the coroutine awaiting Windows Hello also left the native ceremony on screen for up to five minutes; the blocking wait is now interruptible and cancels the native operation.
- **Web: cancelling a `verifyBiometric` awaiting a WebAuthn ceremony now aborts the browser ceremony.** Cancellation released the single-prompt gate while the browser dialog stayed up, so the next call could start an overlapping ceremony the browser rejects — and a cancelled registration could mint a credential whose id was never stored. The `create()`/`get()` ceremonies now carry an `AbortSignal` that is aborted when the awaiting coroutine is cancelled.
- **The Windows Hello bridge no longer assumes a 64-bit JVM.** COM vtable offsets scale with the native pointer size and calls use the stdcall convention, fixing a wrong-function-pointer crash on 32-bit Windows JVMs.
- **JVM `appNamespace` handling is consistent end to end.** The data directory and the key vault normalized the namespace differently, splitting one logical namespace into divergent data/key identities; both now share one canonicalization, with the old identities kept as never-reclaimed migration sources. The namespace copy-forward also preserves file timestamps, so a genuinely newer fallback period is no longer judged stale and stranded, and an existing-but-blank key-vault file is treated as corruption (fail-closed) instead of a healthy empty vault whose "missing" keys let the orphan sweep delete recoverable ciphertext.
- **Apple and Android `getKeyInfo()` report the key's real custody.** Apple now classifies the live Keychain state of the exact alias the read path uses (Secure-Enclave-wrapped vs plain vs Simulator file fallback) instead of inferring from the request and device capability — a legacy plain key under a Secure Enclave request no longer reports hardware isolation. Android (API 31+) verifies the reported tier against the Keystore key's actual security level, so a silent StrongBox-to-TEE fallback at key creation — or a tier-upgrade write that reused a pre-existing TEE key — reports `HARDWARE_BACKED` instead of claiming StrongBox; API 28-30 cannot distinguish StrongBox from TEE and keeps the documented capability inference. Apple debugger detection gains a real `sysctl` traced-flag probe, the jailbreak path list covers modern rootless layouts, and the Android root-package probe ships the manifest `<queries>` it needs on Android 11+.
- **Android 9 and 10 (API 28-29): the default biometric prompt could never be shown.** `BIOMETRIC_STRONG` combined with device-credential fallback is an unsupported authenticator combination on exactly those two API levels, so `PromptInfo.build()` threw before any UI appeared and `canAuthenticate()` reported it unsupported — every `verifyBiometric()` returned `false` and `biometricsAvailable()` reported `false`, with no prompt ever displayed. Those two levels now use the Class 2 (Weak) combination androidx documents for them; the effective bar is unchanged because the same prompt already accepts the device credential, and biometrics-only prompts keep `BIOMETRIC_STRONG` everywhere.
- **Web: two tabs starting for the first time no longer fail their first encrypted write.** The IndexedDB key-create helper rejected on the `ConstraintError` that signals "another context already minted this key" instead of reporting it as a lost race, because a request error bubbles to the transaction and the reject handler beat the completion handler. The loser again adopts the winner's key.
- **A JVM software-fallback session can no longer disarm its own custody marker.** The eager legacy-key sweep treated the marker records themselves as key aliases and migrated them into the OS vault, so on the next launch the conflict guard found no marker and let a fallback-minted key overwrite the live OS-vault key it exists to protect. Markers are now skipped by the sweep.
- **The Android biometric prompt's cancel button is no longer hardcoded English.** The biometrics-only prompt (`allowDeviceCredentialFallback = false`) always rendered a literal "Cancel", so a localized app showed one English control; it now uses the platform's own translated string (overridable via `KSafeBiometrics.defaultCancelLabel`). The Android prompt title also defaults to the app's launcher label instead of a built-in English string.
- **A flow/StateFlow collector can no longer permanently miss a concurrent write (DataStore 1.2.1 race).** DataStore stamps a racing read with an already-incremented version, so its own update filter dropped the write's emission and a freshly started collector froze until some later write. KSafe now merges its own committed states into the snapshot flow, so same-store commits are always observable; the initial value and cross-instance changes still flow from DataStore. The two sources are ordered against each other: a store read that began before a commit could otherwise be delivered *after* it and become the flow's current value, republishing the pre-write content of the whole store as authoritative. Each snapshot now carries the commit sequence that was current when its own read started, and a snapshot behind one already delivered is dropped — a read that started after a commit still wins, which is how a change made outside this storage keeps reaching collectors.
- **A JVM software-fallback session can no longer destroy the live OS-vault key.** Keys minted into the local key file (a `-Dksafe.jvm.keyVault=software` session, a runtime vault degrade, or a vault-less host) now carry a custody marker; the legacy-first key migration refuses to overwrite a different live OS key with a marker-carrying key — both keys are kept, the session continues on the local one, and a security warning asks for resolution. A genuine pre-2.x legacy key (no marker) stays authoritative and still replaces a stale OS copy.
- **Web: an IndexedDB key write is only trusted once its transaction commits.** The key-store helpers resolved on `request.onsuccess`, which fires before the commit — a late abort (quota/IO) could leave committed ciphertext without a persisted key. They now settle on `transaction.oncomplete`/`onabort`.
- **A biometric-invalidated Android key now self-heals on read instead of surfacing as "device locked."** `KeyPermanentlyInvalidatedException` (thrown after biometric enrollment changes) was caught by a broader `InvalidKeyException` handler first and misreported; it is now caught first so the delete-and-recreate recovery actually runs.
- **Reserved internal key names are rejected on write.** Writing a user key in KSafe's reserved namespace (`__ksafe_…`, `encrypted_…`) could collide with internal bookkeeping; such writes now fail fast with a clear error, while reads are unaffected.
- **JVM JSON-fallback `clearAll()` no longer races concurrent writes or leaves the plaintext key file behind.** The wipe now runs on the write consumer through `FileKeyVault.clearAll()`, which also sweeps crash-leftover key temp files.
- **macOS Touch ID on JVM Desktop can no longer crash mid-prompt.** The Objective-C reply block/callback could be garbage-collected while the system prompt was still up (native use-after-free); they are now held alive until the prompt resolves.
- The `fileName` factory argument is documented as the store's **key-isolation boundary**.
- **Reading a `Plain`-written primitive with a complex `@Serializable` type returns the default instead of throwing `ClassCastException`**, and an encrypted (default-mode) write of a `Double`/`Float` `NaN`/`±Infinity` now round-trips like the plain path.
- **A `getFlow`/`getStateFlow` no longer terminates on a single transient storage read error** — the flow resubscribes (matching KSafe's internal collector) instead of crashing the observing scope.
- **Hardening the ENCRYPTED memory policy on a plain→encrypted upgrade**: the pre-encryption plaintext copy is evicted from the in-memory cache rather than lingering for the process lifetime.
- **Fail-closed value classification**: an entry whose metadata is missing or tampered decrypts (and fails to the caller's default) rather than being served as raw bytes.
- **Native/leak fixes**: an Apple Secure-Enclave decrypt no longer leaks `NSData` on the strict-read path; the macOS login-Keychain vault releases the item handle it looks up; the JVM JSON→OS-backed migration retries instead of archiving away data on a transient key-file read outage.
- **Android biometrics**: `biometricsAvailable()` also checks for a usable `FragmentActivity` host, so a `ComponentActivity`-only app no longer gets a false positive; and a Compose persisted state whose write silently fails to persist no longer freezes external observation.
- **A property-delegate, flow, or Compose-state write now inherits the instance's `requireUnlockedDevice`.** `by ksafe(...)`, `asWritableFlow`, `asMutableStateFlow`, and the Compose `mutableStateOf` delegate defaulted to `requireUnlockedDevice = false`, silently ignoring a `KSafeConfig(requireUnlockedDevice = true)` that `put`/`putDirect` already honored — so a delegate-backed key was created without the unlock requirement the app asked for. They now default through the same configured policy, exposed as the new `KSafe.defaultWriteMode`; an explicit `mode` still overrides.
- **Tightening a `HARDWARE_ISOLATED` entry's unlock policy is now natively enforced — via a copy-on-write key, never a destructive swap.** Rewriting an entry from `requireUnlockedDevice = false` to `true` reused the entry's Keystore alias, whose key had been minted without `setUnlockedDeviceRequired` — Keystore key parameters are immutable once minted, so the strict policy existed only in metadata and locked-device reads silently succeeded. Strict per-entry entries now key under a dedicated strict alias variant: the tighten mints its strict key under a fresh alias, commits the rewrite durably, and only then reclaims the relaxed key (guarded so no live entry's alias is ever deleted). Nothing is destroyed before the write is durable — a locked device, a transient Keystore outage, or a crash mid-tighten leaves the previous value fully readable and the write simply fails and can be retried. Entries written by earlier releases keep decrypting under their original alias; their next strict write migrates them. On iOS/macOS the accessibility re-assert is no longer blanket best-effort: a failed *tighten* now fails the write instead of committing strict metadata over a still-relaxed Keychain item; loosening stays best-effort — a failure there leaves the item stricter than declared and retries on the next write.
- **A serializer that throws during encode no longer poisons the key for the process lifetime.** The put paths claimed write ownership and committed routing metadata *before* serializing, so an encode-time `SerializationException` (a custom serializer, or a custom `KSafeConfig.json` missing a contextual type) could leave an entry's intact durable value unreadable through `get`/`getDirect` — while `getFlow` still served it — with no repair until restart. Values are now serialized before any state is touched, so a failed encode leaves the store exactly as it was. Plain writes also encode once instead of twice, so a non-deterministic serializer can no longer put different representations in memory and on disk.
- **A deleted key can no longer be resurrected by a failed same-batch encrypted write.** When a `delete(key)` coalesced with a later encrypted `put(key)` that then failed to encrypt (e.g. a locked device), no removal was committed — the old value survived on disk — yet the delete was acknowledged as success. The delete now fails alongside the write, so a caller is never told a key is gone while its value is still readable.
- **Web `clearAll()` durably erases a store migrated from the pre-appNamespace flat `ksafe_<name>_` layout.** The one-time forward-migration marker guarded only the un-namespaced source; the flat-legacy source was copied forward on every construction, so a namespaced (or default) store re-seeded a secret a prior `clearAll()`/logout had wiped. That copy-forward is now gated by the same one-time marker. `clearAll()` also seals those markers itself: the markers are written best-effort, so a marker write that failed (quota, blocked storage) used to let the retained migration source re-seed the wiped store on the next construction — an explicit wipe now counts every pending migration as done. The one branch that MOVES its source rather than retaining it — a uniquely-named store with no `appNamespace`, which owns the flat prefix alone — was still ungated and was the single branch excluded from that sealing. A copy that fails on quota leaves its entry behind at the source, `clearAll()` is prefix-scoped and cannot see a legacy-prefixed leftover, and the wipe frees the very space the copy needed, so the next page load copied the explicitly-wiped value straight back in. Every legacy copy-forward is now marker-gated and sealed by a wipe, whether it moves its source or retains it.
- **`getOrCreateSecret` no longer hands a special-character key a same-named safe key's secret.** A key whose non-alphanumeric characters collapse to an existing safe key's name (e.g. `"main.db"` vs `"main_db"`), when the safe key was created first, could adopt that safe key's live secret through the legacy-migration probe — silently sharing one passphrase across two logical keys. Safe-key slots are now marked so a colliding sibling can't misread them; the legacy migration of a genuinely distinct special-char secret is unaffected.
- **iOS/macOS: the startup Keychain orphan sweep stands down whenever the store vouches for no encrypted entry, not only when it is completely empty.** The fail-closed guard that exists to stop a partial storage view being mistaken for a genuine post-`clearAll()` state asked whether the DataStore held *any* record. One surviving record of another kind was enough to satisfy it — the reserved rotation-state entry a `rotateKeys()` leaves behind, a plaintext setting the app wrote after the store was lost, ciphertext whose protection metadata did not survive — and then every scoped Keychain account classified as an orphan and had both its Keychain item and its Secure Enclave key deleted, permanently, for entries whose ciphertext was still on disk. The guard now asks whether any encrypted entry is visible at all. The cost is Keychain litter in one benign case (an app that deleted all its encrypted entries but kept plain ones), which is the right side of that trade for key material that cannot be recreated.
- **iOS/macOS: the startup Keychain orphan sweep is skipped for custom-`directory` stores.** The Keychain key namespace is keyed by `fileName`, not `directory`, so a custom-directory instance sharing a `fileName` with a sibling in another location could reap the sibling's live keys as "orphans" (silent loss of `HARDWARE_ISOLATED`/legacy values). `fileName` is now documented as the key-isolation boundary — keep one `KSafe` per `fileName`.
- **Web: an evicted-CryptoKey ciphertext is now reclaimed by the orphan sweep** — its `"web key missing"` error is recognized alongside the other engines' "key not found", so a dead record no longer lingers.
- **A lossy `appNamespace` can no longer collide two different apps onto one store (JVM Desktop + web).** Invalid characters collapse to `_` and the token is capped at 120 chars, so DISTINCT configured ids (`"a/b"` vs `"a?b"`, or two long ids sharing a 120-char prefix) mapped to one namespace token — silently sharing data slots and key custody, each able to read, overwrite, or `clearAll()` the other. A lossy token now carries a short digest of the configured id, keeping such ids apart; already-clean tokens are unchanged (no migration for documented usage), and data and keys written under the old lossy token are migrated forward on upgrade.
- **A deleted key can no longer be resurrected from a retained namespace-migration source (JVM Desktop + web).** The sources that carry keys forward when an `appNamespace` is adopted (the shared JVM OS-vault namespaces; the un-namespaced IndexedDB record on web) are deliberately never deleted — a co-existing sibling may own them live — but a fresh instance's read-fallback would re-copy a key this namespace had deliberately deleted, so old ciphertext backups became decryptable again and new writes reused pre-delete key material. `delete()`/`clearAll()` now leave a persistent per-namespace tombstone that blocks the re-copy, without ever touching the shared source. On web those tombstones are permanent `localStorage` entries competing with the app's own data for the origin's shared quota, and two sources of waste are removed: the alias sweeps no longer enumerate the strict alias spelling on a platform whose write path can never mint one (web strips `requireUnlockedDevice` before an entry's routing is recorded), and after a `clearAll()` a namespaced store writes no further per-alias tombstones at all — the same wipe already seals every key copy-forward, and that one marker does the whole job. Tombstones written before a wipe are still not reclaimable.
- **The JVM `appNamespace` data carry-forward is published as one atomic cohort.** A swallowed copy failure could leave a partial or truncated destination file that counted as "already migrated" and permanently blocked the retry — a data file without its key sidecar — and each file picked its source independently, so a cohort could mix files from different historical stores (including a foreign `.migrated` marker that disables the fallback migration). Files are now staged and renamed into place only when the whole cohort, taken from one source directory, copied successfully; any failure leaves the destination absent so the next launch retries cleanly. The publish itself is ordered and stops at the first failure. The fallback JSON is the trigger file — its mere presence makes the next launch drain it over the store — so it is renamed last, after the two markers that gate that drain, the key sidecar that decrypts it, and the store file whose newer values it must not roll back; a failed rename (a same-directory rename can fail on a file lock from an indexer or scanner) now discards every file still staged instead of skipping just that one. Publishing the fallback data without its `.migration-pending` marker used to re-arm the unconditional "fallback wins" path on the next launch.
- **A JSON-fallback migration whose retry marker couldn't be written can no longer roll newer writes back.** The `.migration-pending` marker is the sole proof that a migration already ran and failed transiently; it was written best-effort, so a second I/O failure (disk full, AV lock) silently dropped it and the next launch re-ran "fallback wins" — overwriting values the session had since written to the OS-backed store with stale fallback data. A 0-byte sentinel is now dropped as a last resort, routing the retry to the conservative baseline that keeps whatever the target already holds.
- **Android: a rare envelope-header collision can no longer get a valid legacy entry deleted.** A pre-software-DEK TEE ciphertext whose random IV happens to spell the DEK envelope header (~2⁻⁴⁰ per entry) is routed down the DEK decrypt path; every failure there already fell back to a legacy-decrypt probe *except* the definitive missing-DEK case, which surfaced as "no encryption key found" — the exact signal the startup orphan sweep deletes on. The missing-DEK failure now probes the legacy path too before surfacing, while the transient "device is locked" signal still propagates unchanged.
- **`getOrCreateSecret` surfaces a malformed stored secret as the documented `IllegalStateException`.** A decryptable-but-not-Base64 value in the reserved slot (the slot overwritten outside `getOrCreateSecret`, or exotic corruption) leaked a raw `IllegalArgumentException` — the type the contract reserves for caller-input validation (blank key / bad size). It now throws the documented existing-but-unreadable failure with the key and remediation in the message, still refusing to overwrite anything; and a malformed *legacy-slot* value is detected before — not after — being copied forward, so it can never poison the canonical slot.

Smaller edge-case hardening: a stored explicit `null` read back with a non-nullable type now returns the requested default instead of deferring a `NullPointerException`; the web engine evicts a deleted key across tabs even on browsers without `BroadcastChannel` (older Safari); a duplicate biometric prompt for two concurrent same-scope calls is suppressed; the Android wait for a `FragmentActivity` host measures its timeout on the monotonic clock, so a wall-clock jump can no longer cut it short or stretch it; a physical `userdebug`/`test-keys` device is no longer classified as an emulator — build type and signing tags are root-capability signals and stay with root detection, so an emulator-`BLOCK` policy no longer trips on engineering hardware for the wrong violation; the snapshot-collector and flow resubscribe-on-failure warnings now include the failing exception, so a permanent underlying error (permissions, corruption) is diagnosable from the logs; `verifyBiometric`'s KDoc no longer claims cancellation returns `false` — cancellation cancels the call and dismisses the pending prompt, as the implementations already did; the web IndexedDB key store reuses one cached connection (dropped on `versionchange`/`close`) instead of opening a fresh one per operation, so a lingering connection can no longer block a future database upgrade; web biometrics tolerate a blocked `localStorage` — a `SecurityError` on the credential-id read now falls back to a fresh registration ceremony instead of throwing out of `verifyBiometric`, and `resetRegistration()` revokes every cached authorization before touching storage so a failing remove can't leave a live prompt-free window; and the Gradle wrapper distribution is now pinned by SHA-256. The web engine always uses AES-256-GCM regardless of `keySize` (now documented), and an encrypted write/read on a non-secure-context page (plain-http, non-`localhost`) now fails with a clear "WebCrypto requires a secure context — serve over HTTPS" message instead of a cryptic `undefined` error, warned once at construction. On the web, KSafe's warning/error diagnostics now route to `console.warn`/`console.error` (not `console.log`), so a failure the developer is hunting is visible under the default DevTools "Errors/Warnings" filter. In that non-secure-context case `KSafe.protectionInfo` (and per-key `getKeyInfo(...).level`) now report the degrade honestly — `effectiveLevel = SOFTWARE` with a `web_crypto_subtle_unavailable` note — instead of a fixed `SANDBOX_PROTECTED`, so a deployment problem is visible in diagnostics (and preflightable via the new `isEncryptionOperational` flag; see Added).

### Compatibility

- An un-rotated store's **existing** entries are byte-identical to 2.2.x — upgrading (and even downgrading again, before any 3.0.0 rotation or strict write) is free for them. Exception: a store using an `appNamespace` whose token contains characters outside `[A-Za-z0-9._-]` is relocated under a collision-safe digested identity on first 3.0.0 launch (the pre-upgrade copy is retained), so a downgraded 2.2.x binary serves that frozen pre-upgrade copy and writes made under it are not carried forward on re-upgrade. Use a clean `appNamespace` token to keep downgrade free.
- **Downgrading after a rotation — or after any 3.0.0 strict `HARDWARE_ISOLATED` write — must be treated as destructive for those entries.** A pre-3.0.0 binary can't resolve their keys (generation-suffixed / strict-variant aliases it doesn't know), and its **startup orphan sweep classifies them as orphans and permanently deletes their rows and metadata** — typically on the first launch. Upgrading back restores access only if the downgraded binary never ran its startup sweep. Treat the first rotation (and the first strict write) as a one-way door, and back up before any planned downgrade.

## [2.2.1] - 2026-07-11

Security-hardening **and feature** release: real biometric prompts on JVM Desktop and the web, and the iOS Simulator now works without Keychain entitlements. It completes the 2.1.3 `requireUnlockedDevice` fix (2.1.3 covered only the direct-read path) and closes a set of multi-instance, multi-tab, and biometric edge cases. **Drop-in upgrade from 2.1.3** — on-disk format is unchanged and existing data keeps working without migration.

> **Supersedes 2.2.0**, published the same day: 2.2.1 is the same release plus a cold-start construction-race fix and four robustness fixes found right after publication. Skip 2.2.0 and depend on 2.2.1 directly.

### Highlights

- **`requireUnlockedDevice = true` is now enforced on every read path, not just `getDirect`.** Flow / StateFlow / Compose-observe reads and the eager `PLAIN_TEXT` memory policy could still serve a strict entry from memory on Apple platforms; 2.2.1 routes strict entries through the native store on all of them. **Upgrade recommended for iOS/macOS apps that observe `requireUnlockedDevice` values via flows.**
...
- **A cold-start construction race that could crash constrained devices is fixed.** KSafe's background coroutines could observe a half-constructed instance on a CPU-starved launch (low-core / busy devices) and crash the first use with a `NullPointerException` — or silently disable external-change observation for the process lifetime.
...
- **`requireUnlockedDevice` was still bypassable on flow-based and `PLAIN_TEXT` reads (Apple).** A strict entry is now always held as ciphertext in the in-memory cache — on every memory policy — and re-decrypted through the engine on each read, so no read path (`asFlow`, `asStateFlow`, `asMutableStateFlow`, `getFlow`, the Compose live-observe states) can serve a strict value from RAM while the device is locked. (Android was already safe — strict entries use the per-call TEE path.)
- **All platforms: a strict (`requireUnlockedDevice`) value is no longer readable from RAM during the optimistic write window.** A strict read returns the default unless the native decrypt succeeds — never the transiently-cached plaintext.
- **Apple: a strict (`requireUnlockedDevice`) key's plaintext no longer lingers in the in-process key cache.** A strict access evicts any cached entry and never repopulates it.
- **Apple: tightening `requireUnlockedDevice` on an existing `HARDWARE_ISOLATED` key had no effect.** Rewriting a key with a stricter policy now re-asserts the Keychain accessibility attribute (best-effort, retried on the next write).
- **Apple: a `HARDWARE_ISOLATED` write no longer fires three `SecItemUpdate` Keychain IPC round-trips on every write.** The accessibility re-assertion runs only on an actual unlock-policy change, tracked per key in-process.
- **Apple (iOS/macOS): the Keychain query dictionaries no longer leak native memory on every operation.** Every `CFBridgingRetain` is now balanced with a `CFRelease` after the `SecItem*` call; strict reads hit the Keychain on every read, so the leak previously grew without bound.
- **Apple (iOS/tvOS/watchOS): the Keychain orphan sweep no longer deletes a key a concurrent write just re-used.** The in-flight guard is re-checked at delete time, not just at classify time. (macOS is unaffected — the sweep is disabled there.)
- **Apple (iOS/tvOS/watchOS): a named instance's Keychain orphan sweep can no longer delete a root instance's live key.** User keys may contain dots, so a root instance's `"vault.token"` and a `KSafe(fileName = "vault")` instance's `"token"` produce byte-identical Keychain accounts; a named instance now reaps only key-ids it can prove it owns.
- **Apple (iOS/macOS): the flow-delegate lazy init no longer serializes every delegate on one process-wide busy-spin lock held across blocking I/O.** Each delegate owns its own reentrant `NSRecursiveLock`; waiters park instead of spinning and unrelated delegates never contend.
- **Android: a `clearAll()` on one instance could permanently orphan a concurrent encrypted write from a co-existing instance on the same file.** The engine now re-persists the data-encryption key (re-wrapping it under a fresh KEK if needed) when it detects the shared DEK was torn down mid-encrypt, so an acknowledged write stays recoverable.
- **Android: the first encrypted read no longer risks an ANR by reading the software DEK on the caller thread.** An already-persisted DEK is warmed into the in-process cache in the background at construction; the prewarm is strictly read-only and never creates or persists a DEK.
- **Android & Apple biometrics: a biometric success that arrives after the caller was cancelled no longer seeds the authorization-session cache.** Seeding happens only while the caller's coroutine is still active, so a cancelled auth can't grant a later call a prompt-free pass.
- **`ksafe-biometrics`: a cached device-credential success could satisfy a biometrics-only call, and cancelling a prompt could strand the next caller.** The authorization-session cache is now keyed injectively by auth strength on both Android and Apple, and a cancelled Android `BiometricPrompt` is dismissed so an orphaned prompt can never rebind to the next caller.
- **All platforms: a construction race could crash the first use on a slow or starved cold start.** The background coroutines (write consumer, snapshot collector, key prewarm) launched before the instance's own fields finished initializing, so on a CPU-starved launch they could observe a half-constructed instance — a `NullPointerException` on first use, or a silently dead external-change collector. Construction side-effects now run strictly after every field is initialized. (Reproduced reliably on 2-vCPU CI runners; load-dependent on real devices.)
- **All platforms: a transient storage-read failure no longer permanently freezes the in-memory cache.** The background snapshot collector previously died on its first error (e.g. a transient `IOException` from DataStore); from then on every read served stale data and no external change was ever observed, until process restart. It now resubscribes with a capped backoff.
- **All platforms: `close()` no longer strands a concurrent suspend `put`/`delete`/`clearAll` mid-flight.** A write still queued (or mid-batch) when the instance is closed now receives a cancellation instead of leaving its caller awaiting forever.
- **All platforms: storing the literal string `"__KSAFE_NULL_VALUE__"` as a plain value no longer reads back as `null`.** The one string that collides with the internal null marker is escaped on write and restored on read; every other string is stored byte-identical. (Encrypted values were never affected.)
- **A deleted value could be resurrected in memory, or a newer write's metadata clobbered, during the post-commit cache repair (all platforms).** The repair's rollback is now atomic-per-value: an acknowledged delete is never resurrected and a newer writer's state is never clobbered. (In-memory only; disk was always correct.)
- **Core: a `delete()` racing the post-commit repair no longer resurrects orphan metadata that breaks `getOrCreateSecret`/`getKeyInfo` for the rest of the session.** The repair drops exactly the literals it inserted when it lost ownership and no value is cached for the key, so delete-then-recreate secret rotation works again. (In-memory only; disk was correct.)
- **Core: the post-commit repair's Plain branch no longer resurrects orphan protection metadata for a key deleted mid-repair.** The Plain branch applies the same orphan-drop as the Encrypted branch, and the plain-put path preserves the value-before-protection ordering the cleanup relies on.
- **Core: a failing encrypted write no longer cross-contaminates a superseded same-key awaiter in the same batch.** Only the final (failing) op per key fails; superseded same-key ops absorbed into the coalesced write complete normally.
- **All platforms: a transient decrypt failure on a locked device no longer throws out of `getDirect`, the value delegate, or StateFlow/Compose seeding.** The non-suspend read path returns the default on a transient failure; the suspend `get()` path still throws so coroutine callers can await unlock and retry (the intentional, documented asymmetry).
- **All platforms: an external write is no longer masked by a stale plaintext side cache under `LAZY_PLAIN_TEXT` (the default) / `ENCRYPTED_WITH_TIMED_CACHE`.** Merging an external change now evicts the side-cache entry when the ciphertext actually changed, and mirrors external deletes, so the next read re-decrypts the fresh value.
- **All platforms: the flow property delegates (`asFlow` / `asStateFlow` / `asWritableFlow` / `asMutableStateFlow`) no longer double-initialize under concurrent first access.** Lazy flow construction uses double-checked init, so concurrent first access returns exactly one canonical flow and starts exactly one observer.
- **All platforms: `lazyLoad = true` no longer skips the one-time startup cleanup.** The legacy-key sweep, the orphaned-ciphertext sweep, and the Apple access-policy migration now also run once on a lazy instance's first access, deferred to a background scope so they never block the read.
- **All platforms: a pre-2.0 flat plaintext user key that merely begins with `ksafe_` is no longer dropped on upgrade.** The internal-key filter matches only the genuine internal prefixes — the reserved `__ksafe_*` namespace and the web engine's `ksafe_key_*` key store.
- **All platforms: an encrypted `getStateFlow` / `getFlow` no longer runs its per-emission decrypt on the collector's dispatcher, avoiding a main-thread ANR on Android.** The flow chain ends with `flowOn` over a background dispatcher on Android/JVM/Apple (a no-op on single-threaded web), so the blocking keystore IPC runs off the collector's dispatcher.
- **Compose / StateFlow: a stale flow emission that races the setter no longer permanently clobbers a user write.** The matched-echo branch re-applies the user's value, self-healing the race; in the non-raced case that re-apply is a no-op.
- **Compose / Core: live external-change observation could stop reflecting updates permanently after an idempotent write.** The write-echo guard now tracks the storage-synced baseline and is not held when a write nets to no change (e.g. `value = X` when already `X`, or an `A→B→A` toggle).
- **Compose (`mutableStateOf` / `rememberKSafeState`): the one-shot cold-start self-heal can no longer permanently clobber a concurrent user write.** The setter arms the write-echo guard before publishing, and the self-heal re-checks the guard after applying and re-applies the user's value if a write raced in.
- **`asMutableStateFlow` / `KSafeMutableStateFlow`: concurrent writers can no longer corrupt the write-echo guard.** The write-side transition runs under a per-instance lock, so a concurrent writer never observes a half-updated guard state; `persist()` stays outside the lock.
- **`getOrCreateSecret` no longer surfaces a raw keystore exception on a locked device.** A transient decrypt failure routes to the documented refuse-to-rotate `IllegalStateException` when the secret exists, so a locked device yields a "temporarily unavailable — retry" message and the secret is never rotated.
- **`getOrCreateSecret` now derives an injective storage key, so distinct secret names can no longer silently collide into one secret.** A name already in `[A-Za-z0-9_]` keeps its historical slot; any other name is hex-encoded under a distinct prefix that can never collide with a plain-name slot, with its old secret migrated forward non-destructively on first access.
- **JVM/Desktop: the `-Dksafe.jvm.keyVault=software` opt-out could let the startup orphan sweep delete recoverable data.** Keys that live only in the OS vault now read as "unavailable" under the opt-out, so the sweep preserves their ciphertext while new keys still mint into the software store.
- **macOS/Desktop (JVM): the login-Keychain key vault now fails closed on `errSecDuplicateItem`.** A duplicate error from the delete-then-add upsert means the new key was never stored; `put` now throws on any non-success status instead of reporting a silently-lost write.
- **JVM/Desktop: the software key vault (`FileKeyVault`) now fsyncs the parent directory after its atomic key-file write**, so a crash right after the move can't lose the rename and read back as "no keys yet" (which would let the orphan sweep delete every encrypted entry).
- **JVM/Desktop: a crash mid key-file write can no longer leave a plaintext copy of the key map on disk.** The software key vault's atomic write goes through a temp file that holds the full key map; a process killed before the atomic move orphaned it, and nothing ever deleted it. Stale temp files are now swept on the next write, and `clearAll()` removes them too (still sparing a sibling safe's files).
- **JVM/Desktop: no residual live fallback secrets survive migration or `clearAll()`.** The migration's copy path deletes its source after a successful copy, and `clearAll()` also sweeps any live fallback JSON/key files left behind (matched by exact name, so a sibling safe's files are spared).
- **JVM/Desktop: `clearAll()` now also deletes the DataStore corruption-quarantine copies** (`<base>.preferences_pb.corrupt-<ts>`), which still hold decryptable ciphertext.
- **JVM/Desktop: a failed archive step in the JSON→OS-backed migration no longer silently reverts every later write on every launch.** The archive step guarantees a durable done-marker (rename → copy → 0-byte sentinel as a last resort), so the migration can never re-drain stale fallback values over the user's newer writes.
- **JVM/Desktop: a corrupt `.migration-pending` file no longer makes the JSON→OS-backed migration roll back the user's writes.** A present-but-corrupt pending file is treated as a retry with an unknown baseline — keys the target already holds are conservatively kept — and the file is written atomically so a partial write can't corrupt it.
- **JVM/Desktop: adding `config.appNamespace` on upgrade no longer orphans the OS-vault keys.** Keys written before the namespace was configured are migrated forward from the stable default namespace on first access, so existing encrypted data survives the upgrade.
- **JVM/Desktop: adding `appNamespace` no longer lets one instance destructively steal a co-existing no-namespace instance's OS-vault key.** Recovery from the shared default namespace is copy-only; the destructive move is reserved for a derived legacy namespace that has no live owner.
- **JVM/Desktop: an app that upgraded from 2.1.0/2.1.1 (launcher-derived OS-vault namespace) and newly set an explicit `appNamespace` no longer orphans its keys.** Key recovery probes the full set of candidate legacy namespaces (the derived one and the stable default), migrating from the first that holds the key.
- **JVM/Desktop: a transient "vault unavailable" during legacy-namespace recovery no longer reads as a genuine miss.** The probe read rethrows the "vault unavailable" signal instead of collapsing it to null, so the orphan sweep preserves the ciphertext until the store is reachable again.
- **Web (Kotlin/JS + Wasm): `appNamespace` did not isolate the localStorage data store.** The data namespace is now `ksafe.<appNamespace>@<fileName>:` when an `appNamespace` is set (prefix-free — `@` is outside both the fileName alphabet and the sanitized namespace charset), with a one-time migration carrying forward data written under the previous un-namespaced prefix. The default (no `appNamespace`) layout is unchanged.
...
- **Web: values written with `requireUnlockedDevice = true` are now readable.** The flag cannot be hardware-enforced in a browser, so the web factory strips it instead of routing strict reads to a blocking decrypt the async-only WebCrypto engine cannot serve.
- **Web (JS/Wasm): `getOrCreateSecret` with a secret larger than 64 KB no longer throws.** The web random source fills in ≤64 KB chunks (WebCrypto's `getRandomValues` view limit).
...
- The iOS Keychain memory-leak regression test uses a strict threshold on the non-throwing `deleteKey` path and a separate loose threshold only on the throwing `decrypt` path, whose Kotlin/Native 2.3+ exception stack traces inflate peak RSS independently of the leak under test.
