...
- **Android: root detection no longer misses rooted devices on modern Android.** `isDeviceRooted()` silently returned `false` on rooted emulators and `userdebug` builds — including stock Google-APIs emulator images, which ship `su` and allow `adb root` — because its file and `getprop`-subprocess probes are blocked by the post-Android-11 app sandbox, and its build-tag check matched only `test-keys` (not the `dev-keys` of current Google emulator images). Detection now also keys off the sandbox-readable `Build.TYPE` / `Build.TAGS` and reads system properties in-process. Matters for anyone relying on `KSafeSecurityPolicy.Strict` / `WarnOnly` or a custom root policy. See **Fixed**.
...
- **Android: rooted / `userdebug` devices are now detected on modern Android (API 30+).** `SecurityChecker.isDeviceRooted()` silently returned `false` on rooted emulators and engineering builds — including stock Google-APIs emulator system images, which ship `su` at `/system/xbin/su` and allow `adb root`. Three independent causes, all stemming from the post-Android-11 app sandbox: (1) the `su` / Magisk / busybox path probes call `File("/system/xbin/su").exists()`, which SELinux denies the `untrusted_app` domain — the binary exists but can't be `stat`'d, and the surrounding `try/catch` swallowed the denial as `false`; (2) the build-tags check matched only the literal `"test-keys"`, but current Google emulator images are signed `dev-keys`; (3) `ro.debuggable` / `ro.secure` were read with `Runtime.exec("getprop …")`, a subprocess SELinux blocks for app processes. Detection now adds a sandbox-proof signal — `userdebug` / `eng` build types and `test-keys` / `dev-keys` signing, read from the public `Build.TYPE` / `Build.TAGS` (extracted as the pure, testable `isRootIndicatingBuild(type, tags)`) — and reads dangerous props in-process via reflective `android.os.SystemProperties.get` instead of a `getprop` subprocess. Retail `user` / `release-keys` devices and Play-Store emulators remain reported as not-rooted; the file / Magisk / package probes are retained for older devices. With the default `IGNORE` policy this only changes what `isDeviceRooted()` reports; under `Strict` (root → `BLOCK`) a rooted device or emulator is now correctly blocked. iOS is unchanged — a simulator is intentionally not treated as jailbroken.
...
- **Android root detection now has direct coverage** (`AndroidSecurityCheckerTest`, 4 cases, `androidDeviceTest` instrumented source set): the pure `isRootIndicatingBuild(type, tags)` predicate flags `userdebug` / `eng` and `test-keys` / `dev-keys` while leaving `user` / `release-keys` alone, plus a device assertion that any root-capable image the suite runs on (e.g. a Google-APIs emulator) is reported rooted by `isDeviceRooted()`. Regression guard for the false negative above.
