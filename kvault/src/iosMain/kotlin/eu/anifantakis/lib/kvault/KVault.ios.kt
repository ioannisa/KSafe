...
actual class KVault {
    ...
    companion object {
        ...
        private const val SERVICE_NAME = "eu.anifantakis.kvault"
        private const val KEY_PREFIX = "eu.anifantakis.kvault."
        private const val INSTALLATION_ID_KEY = "kvault_installation_id"
    }
    ...
    internal val dataStore: DataStore<Preferences> = PreferenceDataStoreFactory.createWithPath(
        ...
        produceFile = {
            ...
            requireNotNull(docDir).path.plus("/eu_anifantakis_kvault_datastore.preferences_pb").toPath()
        }
    )
    ...
    suspend fun ensureCleanupPerformed() {
        if (!cleanupPerformed) {
            ...
            try {
                ...
                println("KVault: Failed to cleanup orphaned keychain entries: ${e.message}")
            }
        }
    }
