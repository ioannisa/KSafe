...
- **Off the blocking startup path.** The automatic `KSafeKeyRotationPolicy.MaxAge(...)` policy runs its age check **once per startup, in the background**. It never blocks startup or reads — a store that becomes due is rotated behind the scenes while normal reads keep serving. The default policy is `Never`, so no fresh rotation is started by default; since 3.1.0, background lifecycle work under `Never` may still finish a pass marked `r:1` after a crash or consume one attempt from a bounded `rp:N` next-instance budget left by normally skipped work (3 attempts by default, 0 disables). A marker-less 3.0.0 record is only adopted as completed on its first 3.1.0 startup.
