...
- `data class KSafeConfig(keySize, requireUnlockedDevice, json, appNamespace, keyRotationPolicy)`.
...
- `keyRotationPolicy: KSafeKeyRotationPolicy = KSafeKeyRotationPolicy.Never` (3.0.0+) — when to rotate encryption keys automatically. `Never` (default) means rotation is on-demand only (`ksafe.rotateKeys()`); `MaxAge(Duration)` runs one background check shortly after startup and, if the current key generation is older than the policy, kicks off a full rotation. It never blocks startup or reads. See `KSafeKeyRotationPolicy.kt`.

**Why:** centralises the "things you configure once at construction time" — key size, the default unlock policy, the `Json` instance for `@Serializable` types, the app namespace that keeps one app's desktop/web keys from colliding with another's in the shared per-user secret store, and the automatic key-rotation policy. Separating config into its own class keeps the factory signatures readable, and it lets users swap in a `Json` with `@Contextual` serializers for `UUID`, `Instant`, etc.
...
- `sealed interface KSafeKeyRotationPolicy`:
  - `data object Never` — the default. No automatic rotation; rotate on demand with `ksafe.rotateKeys()`.
...
**Why:** key material on every KSafe platform is hardware- or OS-protected and doesn't expire, so automatic rotation is an opt-in hygiene/compliance policy rather than a security necessity — hence `Never` by default. The policy never blocks startup or reads; entries it can't rotate this launch (e.g. a strict entry on a locked device) stay readable under their retained old key. A manual `rotateKeys()` picks them up immediately; the automatic `MaxAge` scheduler only re-attempts them after another full `maxAge`, because each rotation restamps the generation-birth time and restarts the age clock.
...
**Why:** rotation is resumable, not all-or-nothing. A `skipped`/`failed` entry keeps decrypting under the generation recorded in its own metadata (that key is retained until nothing references it), and the next `rotateKeys()` picks it up again.
...
- `rotateKeys(): KSafeRotationResult` (3.0.0+) — the whole-store rotation. Single-flighted via `rotationInFlight`; a second concurrent call fails fast. Persists the generation bump **first** through the write consumer (so every write ordered after it already encrypts under the new generation and the candidate set only shrinks), then re-encrypts each older-generation/legacy entry via a per-entry `PendingWrite.Rotate` that commits under a CAS on the entry's stored ciphertext — a concurrent user write always wins and is never clobbered. Values are preserved (only the wrapping key changes); strict entries on a locked device and superseded entries are counted `skipped`, a transient key-store outage is `skipped` not `failed`, and superseded master generations nothing references are swept afterward. No journal: a mixed-generation store stays fully readable and the next call resumes.
