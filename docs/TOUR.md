...
- **`androidDeviceTest/`** (renamed from `androidInstrumentedTest/` under the AGP 9 `com.android.kotlin.multiplatform.library` plugin) — `AndroidKSafeTest`, `AndroidKSafeNullableDefaultTest`, `AndroidEncryptionProofTest`, `AndroidConcurrencyTest`, `AndroidSecurityCheckerTest`, `NullFilenameTest`, **`AndroidStorageLocationTest`** (locks in the 2.0 cache-key change: same `fileName` + different `baseDir` → isolated DataStores instead of DataStore's "multiple active instances" crash), and **`AndroidSoftwareDekTest`** (2.1.2 — verifies the software-DEK fast path: relaxed-`DEFAULT` round-trip + DEK header, cross-version read of pre-2.1.2 TEE ciphertext, `HARDWARE_ISOLATED`/locked-master staying on the TEE path, and `deleteKey`/orphan handling). Require a device or emulator.

### Sister-module tests

- **`:ksafe-biometrics/commonTest/`** — `BiometricAuthorizationDurationTest` (the data class).
- **`:ksafe-biometrics/jvmTest/`** — `KSafeBiometricsJvmTest` pins the opt-out (`ksafe.biometrics.jvm.prompts=off`) no-op contract — the pre-2.2.0 behavior and what JS/WasmJS still do. `DesktopBiometricsTest` covers the 2.2.0 desktop-prompt dispatch via a test seam (denial propagates as `false`, success seeds the strength-keyed authorization cache) plus the WinRT pinterface-GUID computation the Windows Hello bridge depends on, verified against published reference GUIDs; its `livePrompt` test (opt-in via `-Dksafe.biometrics.live=1`) pops a real system prompt.
