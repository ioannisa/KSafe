...
- **PLAIN_TEXT:** Discouraged. Eagerly decrypts every encrypted entry at cold start, which can push first-read latency into ANR territory on Android with thousands of encrypted keys. Only useful when you need all values plaintext-resident and O(1) from the very first read. Note: an entry whose decrypt fails at cold start is silently dropped from the cache (no startup exception) and reads back as its default.
- **ENCRYPTED:** Maximum security — stores ciphertext in RAM, decrypts on every read. Nothing plaintext sits in RAM at rest.
- **ENCRYPTED_WITH_TIMED_CACHE:** Like `LAZY_PLAIN_TEXT` but with a TTL — the cached plaintext stops being reused after a configurable duration. There is no background sweeper — after the TTL an expired entry is ignored (not proactively zeroized) on the next access and stays referenced in RAM until that key is next read (which re-decrypts and overwrites it), written, deleted, or until `clearAll()`. The TTL bounds reuse, not the in-RAM lifetime. Use when you need plaintext briefly for UI bursts but don't want it to live in RAM permanently.
...
The in-RAM exposure is the same as `PLAIN_TEXT` *for keys you've actually read at least once*: the plaintext stays cached until process exit (or `clearAll()`). Keys you never read are never stored in the plaintext cache. (A one-time startup integrity sweep may decrypt an entry transiently to detect orphaned ciphertext, but that plaintext is immediately discarded, never cached.) If you need ciphertext-at-rest semantics for sensitive values, opt into `ENCRYPTED` or `ENCRYPTED_WITH_TIMED_CACHE` for those instances explicitly.
...
// Keys you never read never enter the plaintext cache — their plaintext is never retained in RAM.
```
...
KSafe(
    ...
    directory: String? = null                 // override the default DataStore directory (see docs/SETUP.md)
)
...
val ksafe = KSafe(
    ...
    config = KSafeConfig(
        ...
        keyRotationPolicy = KSafeKeyRotationPolicy.Never  // Never (default) or MaxAge(Duration) — see docs/KEY_ROTATION.md
    )
)
```
...
`requireUnlockedDevice` applies only to encrypted writes; `KSafeWriteMode.Plain` intentionally ignores it. It maps to `setUnlockedDeviceRequired(true)` (Android API 28+) and `WhenUnlockedThisDeviceOnly` (Apple), and is a no-op on JVM (software keys) and web (no device-lock concept).

For the full per-platform truth table, the per-key metadata shape, and the locked-device error/queue behavior, see [docs/SECURITY_MODEL.md](SECURITY_MODEL.md).
