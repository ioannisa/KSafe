...
- **`JvmSoftwareEncryption`** — AES-256-GCM via `javax.crypto` for the payload, but the AES key itself is held by an **OS secret store** through the `JvmKeyVault` abstraction (`jvmMain/internal/keyvault/`), selected per host via JNA:
  ...
  - **No-`sun.misc.Unsafe` fallback (2.1.1+)** — distinct from the key-vault degrade above. When `sun.misc.Unsafe` is missing entirely at construction (the typical trimmed Compose Desktop release distributable built without `modules("jdk.unsupported")`), `buildJvmKSafe` swaps the *whole* backend rather than crash inside DataStore: storage → `DataStoreJsonStorage` (Jetpack `datastore-core` + a custom JSON serializer, no Preferences protobuf), keys → `FileKeyVault` (software, local JSON file at `0700`). Data persists; `protectionInfo` reports `SOFTWARE`. When the module is later added, a one-time `migrateJsonFallbackToOsBacked` pass re-encrypts the fallback data under a fresh OS-backed key on first launch (fallback values win) and archives the source files to `*.migrated`.
