...
- **`JvmSoftwareEncryption`** — AES-256-GCM via `javax.crypto` for the payload, but the AES key itself is held by an **OS secret store** through the `JvmKeyVault` abstraction (`jvmMain/internal/keyvault/`), selected per host via JNA:
  ...
  - **Fallback (construction-time and runtime)** — when no OS store is reachable (headless Linux with no keyring, a JNA link failure during the construction-time self-test, …) it degrades to the legacy scheme: the key Base64-encoded inside the same DataStore as the data, under `~/.eu_anifantakis_ksafe/` enforced at POSIX `0700`, plus a one-time `System.err` security warning. **2.1.1+** also degrades on a *runtime* `LinkageError` from any vault call (`JvmKeyVaultProvider.degradeToLegacy`) — e.g. a JNA call that fails mid-process (a missing native lib, an unavailable keychain). The encrypt path catches it, flips the active vault, retries on legacy, and the next read of `KSafe.protectionInfo` reports the degrade.
  - **No-`sun.misc.Unsafe` fallback (2.1.1+)** — distinct from the key-vault degrade above. When `sun.misc.Unsafe` is missing at construction (a trimmed Compose Desktop release distributable without `modules("jdk.unsupported")`), `buildJvmKSafe` swaps the whole backend rather than crash: storage → `DataStoreJsonStorage` (datastore-core + a JSON serializer, no protobuf), keys → `FileKeyVault` (software, `0700`). Same `SOFTWARE` tier as the no-keyring case; `migrateJsonFallbackToOsBacked` re-encrypts it forward (fallback values win) when the module is added. Full risk + mechanism: [JVM_PROTECTION.md](JVM_PROTECTION.md#compose-desktop-release-distributables-jdkunsupported).
  ...
  - **Legacy key is authoritative.** The OS secret store is per-OS-user and long-lived (it outlives the DataStore across reinstalls/data-clears/mixed-version runs), so it can already hold a *stale* key under the same alias. Key resolution therefore treats a present legacy DataStore key as authoritative — it provably encrypted the on-disk ciphertext — and migrates it, **overwriting** any stale OS-store entry (then verify, then scrub); the OS store is trusted only when there is no legacy key. (The pre-2.1 logic trusted the OS store first and only migrated when it was empty, so a stale entry shadowed the real key and silently reset every encrypted value — `Plain` values, needing no key, survived. Regression-tested against a real Keychain/DPAPI/Secret Service pre-seeded with a stale key.)
  - **Per-app isolation.** Unlike Android/iOS keystores (OS-sandboxed per app), the JVM desktop secret store is **per-OS-user, shared by every process** — two apps using the same `fileName` collide on the same key. `KSafeConfig.appNamespace` namespaces the OS-store *destination* (Keychain service / DPAPI blob prefix / Secret Service attribute), resolved as: explicit value → `-Dksafe.appNamespace` → env `KSAFE_APP_NAMESPACE` → a stable best-effort id from the app's launcher → `"shared"`. The legacy `ksafe_key_` DataStore layout is **not** namespaced (it's the frozen ≤ 2.0 migration source), so namespacing never strands existing data.
  - See [JVM_PROTECTION.md](JVM_PROTECTION.md) for the platform-by-platform deep dive (what DPAPI / Keychain / Secret Service actually are, threat model per OS, the fallback warning, the opt-out, the self-test).
  - The active vault selection (and any fallback) is surfaced to consumers through the cross-platform `KSafe.protectionInfo` API — see [PROTECTION_INFO.md](PROTECTION_INFO.md) for the `KSafeProtectionLevel` scale, the per-platform truth table, and consumer gating / telemetry / UI patterns.
...
## Master AES keys and the v2 envelope
...
- `keyAlias(userKey) → "$fileName?:$userKey"` (or `"$KEY_ALIAS_PREFIX.$fileName?.$userKey"` on Android) — used for `HARDWARE_ISOLATED` entries, identical to the pre-2.0 scheme.
- `masterAlias(requireUnlockedDevice) → "$fileName?:__master__[_locked]"` — used for every v2 `DEFAULT` entry.

`KSafeCore.aliasForWrite(userKey, protection, requireUnlockedDevice)` returns the master for `DEFAULT`, the per-entry alias for `HARDWARE_ISOLATED`. `aliasForRead(userKey, protection)` does the inverse, consulting `encMetaMap` (below) to handle pre-v2 ciphertext that still uses per-entry keys.

**`encMetaMap` — what the read path needs to know.** Every encrypted entry's on-disk metadata records the envelope version (`v=1` legacy, `v=2` master-key) and the per-entry `requireUnlockedDevice` flag. `KSafeCore` mirrors this into an in-memory `KSafeConcurrentMap<EncMeta>` populated during cold-start preload. The read path consults it to pick the master vs. per-entry alias, so the engine can serve both v1 (pre-2.0) and v2 (2.0+) ciphertext side by side without a separate migration pass — old entries decrypt under their per-entry key, then the next write rewrites them as v2.

**`prewarmMasterKeys()`.** Cold start runs a single `encryptSuspend(masterAlias, emptyBytes)` against each master alias right after the first snapshot completes. The probe materialises the Keystore/Keychain handle eagerly so the very first user-driven `put` doesn't pay the key-creation latency on the foreground path. Idempotent — a second cold start finds the handle already in place and the probe is effectively free.
...
**Where to read the code:** `KSafeCore.kt` — `aliasForRead` / `aliasForWrite`, `prewarmMasterKeys`, the `encMetaMap` field and its population in `loadInitialSnapshot`, and the `EncMeta` data class. The platform-shell constructors that supply `masterAlias` are in `KSafe.{android,apple,jvm,web}.kt`.

## Key custody across platforms: the KEK / DEK model

The v2 envelope settles *how many* keys a safe uses (one master per datastore). A separate question is *how that master key actually performs each encrypt/decrypt* — and the answer differs per platform, because each platform's secure store exposes the key differently. This is where the **KEK / DEK** distinction matters:
