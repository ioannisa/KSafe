...
- **`JvmSoftwareEncryption`** — AES-256-GCM via `javax.crypto` for the payload, but the AES key itself is held by an **OS secret store** through the `JvmKeyVault` abstraction (`jvmMain/internal/keyvault/`), selected per host via JNA:
  ...
  - **Fallback (construction-time and runtime)** — when no OS store is reachable (headless Linux with no keyring, a JNA link failure during the construction-time self-test, …) it degrades to the legacy scheme: the key Base64-encoded inside the same DataStore as the data, under `~/.eu_anifantakis_ksafe/` enforced at POSIX `0700`, plus a one-time `System.err` security warning. **2.1.1+** also degrades on a *runtime* `LinkageError` from any vault call (`JvmKeyVaultProvider.degradeToLegacy`) — the canonical case is a Compose Desktop release distributable built without `modules("jdk.unsupported")`, where the self-test passes but the first real JNA call throws `NoClassDefFoundError: sun/misc/Unsafe`. The encrypt path catches it, flips the active vault, retries on legacy, and the next read of `KSafe.protectionInfo` reports the degrade.
...
- `keyAlias(userKey) → "$fileName?:$userKey"` (or `"$KEY_ALIAS_PREFIX.$fileName?.$userKey"` on Android) — used for `HARDWARE_ISOLATED` entries, identical to the pre-2.1.0 scheme.
