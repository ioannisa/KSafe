...
- **`AndroidKeystoreEncryption`** — AES-GCM (`BITS_256` by default; `BITS_128` via `KSafeConfig.aesKeySize`) with hardware-backed keys (StrongBox when requested + available). Keys are handles; the bytes never leave the TEE.
...
- The store's current generation is persisted as a reserved entry (`__ksafe_keygen__`), written
  through the same coalescer as every other op. In 3.1.0 its normal shape is
  `{"g":N,"ts":…,"r":0}` with optional `"rp":N`: `ts` is the generation birth time measured
  by `MaxAge`, `r` is the pass lifecycle (`0` completed, `1` in progress), and `rp` is the
  bounded number of automatic next-instance retries remaining after retryable skips. `SetKeyGeneration`
  writes generation bump + `r:1` atomically, and `CompleteKeyRotation` changes it to `r:0`
  only if the persisted store is still at that generation. With skipped work it also writes
  the configured budget (3 by default); the current instance stops, and the next one
  generation-CASes `r:0,rp:N` to `r:1,rp:N-1` before retrying the same generation without
  changing `ts`. That decrement is durable, so a crash cannot refill the budget; `r:1,rp:0`
  means the final claimed attempt still needs crash recovery but cannot arm another retry.
  If `MaxAge` is already due on that run, its fresh-generation rotation takes precedence.
  A 3.0.0 record has no `r`; startup adopts it once as completed without doing rotation work.
...
readable**. In 3.1.0+, the persisted `"r":1` lifecycle marker makes the next KSafe instance
resume the same target generation in the background, even under `Never`; it becomes `"r":0`
only after the entry pass and master sweep finish. There is no per-entry journal or rollback
log, and no unreadable intermediate state. Because 3.0.0 had no `r`, absence is never treated
...
- Store key generation: `__ksafe_keygen__` (a single reserved
  `{"g":N,"ts":…,"r":0|1,"rp":N?}` entry recording the current generation, birth time,
  completed/in-progress lifecycle, and optional remaining next-instance retry count;
  released 3.0.0 records omit `r`/`rp` and are adopted as completed on their first 3.1.0
  startup)
