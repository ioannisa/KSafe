...
| **JVM/Desktop** | `~/.eu_anifantakis_ksafe/eu_anifantakis_ksafe_datastore[_<fileName>].preferences_pb`, with POSIX `0700` permissions. The encryption *key* lives in the OS secret store (Windows DPAPI / macOS Keychain / Linux libsecret), not in this file — only the no-keyring fallback keeps the key here. (A trimmed release distributable without `modules("jdk.unsupported")` instead writes `…ksafe.json` + `…ksafe-keys.json` in this dir — same `SOFTWARE` tier; see [JVM_PROTECTION.md](JVM_PROTECTION.md).) |
