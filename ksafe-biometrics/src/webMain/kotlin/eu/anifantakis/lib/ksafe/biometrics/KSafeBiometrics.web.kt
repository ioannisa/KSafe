...
object KSafeBiometricsWeb {
    ...
    fun resetRegistration() {
        val abandoned = runCatching { localGet(WEBAUTHN_CREDENTIAL_ID_KEY) }.getOrNull()
    ...
    val isRegistered: Boolean
        get() = runCatching { localGet(WEBAUTHN_CREDENTIAL_ID_KEY) }.getOrNull() != null
    ...
    val registeredTitle: String?
        get() = runCatching { localGet(WEBAUTHN_REGISTERED_TITLE_KEY) }.getOrNull()
}
...
internal var webBioLocalGetOverrideForTest: ((key: String) -> String?)? = null
internal var webBioLocalSetOverrideForTest: ((key: String, value: String) -> Unit)? = null
internal var webBioLocalRemoveOverrideForTest: ((key: String) -> Unit)? = null

private fun localGet(key: String): String? {
    val override = webBioLocalGetOverrideForTest
    return if (override != null) override(key) else webBioLocalGet(key)
}

private fun localSet(key: String, value: String) {
    val override = webBioLocalSetOverrideForTest
    if (override != null) override(key, value) else webBioLocalSet(key, value)
}
...
private suspend fun runWebAuthnGate(allowDeviceCredentialFallback: Boolean, title: String?): WebAuthnGateResult {
    ...
    val credentialId = runCatching { localGet(WEBAUTHN_CREDENTIAL_ID_KEY) }.getOrNull()
    ...
    return when {
        ...
        outcome.startsWith("registered:") -> {
            runCatching { localSet(WEBAUTHN_CREDENTIAL_ID_KEY, outcome.removePrefix("registered:")) }
            // Absent title -> slot removed, so a legacy record and a title-less one both read null.
            runCatching {
                if (title != null) localSet(WEBAUTHN_REGISTERED_TITLE_KEY, title)
