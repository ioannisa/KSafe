...
/**
 * Locks in: the authenticator set KSafe asks for is one androidx accepts on this API level.
 * androidx rejects `BIOMETRIC_STRONG or DEVICE_CREDENTIAL` on API 28-29, where `PromptInfo.build()`
 * throws and `canAuthenticate` answers unsupported. Nothing is ever displayed, so this runs
 * unattended; the two API-range tests are mutually exclusive, so one of them always reports skipped.
 */
...
class AndroidDeviceCredentialAuthenticatorsTest {
    ...
    /** Mirrors the production prompt: a title, and no negative-button text (forbidden with DEVICE_CREDENTIAL). */
    ...
    fun chosenAuthenticators_areAcceptedByAndroidx_onThisDevice() {
        ...
        // The probe must ask about the set the prompt is built from, or it answers for another one.
    ...
    /** API 28-29 only: the Class 2 branch is load-bearing because androidx really rejects Class 3 here. */
    @Test
