...
// `-PksafeTestJdk=<major>` launches the test JVMs of every module on that JDK through a
// toolchain launcher, while Gradle itself keeps running on the JDK that started it (9.x needs
// 17+). The CI job that guards the advertised JDK 11 floor passes 11; the foojay resolver in
// settings.gradle.kts provisions the JDK when the machine doesn't have one.
val ksafeTestJdk = providers.gradleProperty("ksafeTestJdk")
if (ksafeTestJdk.isPresent) {
    val testJdkVersion = JavaLanguageVersion.of(ksafeTestJdk.get())
    subprojects {
        // KGP applies `jvm-toolchains` itself, but the root script is evaluated first.
        val toolchains = extensions.findByType(JavaToolchainService::class.java)
            ?: run {
                pluginManager.apply("org.gradle.jvm-toolchains")
                extensions.getByType(JavaToolchainService::class.java)
            }
        val launcher = toolchains.launcherFor { languageVersion.set(testJdkVersion) }
